...
class TextLine {
    ...
    int getOffsetToLeftRightOf(int cursor, boolean toLeft) {
        ...
        if (cursor == 0) {
            ...
            if (runIndex != runs.length) {
                int i2;
                boolean runIsRtl2 = (runLevel & 1) != 0;
                advance = toLeft == runIsRtl2;
                if (advance) {
                    i2 = runLimit;
                } else {
                    i2 = runStart;
                }
                if (!(cursor == i2 && advance == trailing)) {
                    newCaret = getOffsetBeforeAfter(runIndex, runStart, runLimit, runIsRtl2, cursor, advance);
                    if (advance) {
                        i2 = runLimit;
                    } else {
                        i2 = runStart;
        ...
        while (true) {
            ...
            if (otherRunIndex >= 0 && otherRunIndex < runs.length) {
                ...
                if (newCaret == -1) {
                    ...
                    newCaret = getOffsetBeforeAfter(otherRunIndex, otherRunStart, otherRunLimit, otherRunIsRtl, i3, advance);
                    if (!advance) {
