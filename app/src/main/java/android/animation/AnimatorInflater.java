...
public class AnimatorInflater {
    ...
    private static PropertyValuesHolder loadPvh(Resources res, Theme theme, XmlPullParser parser, String propertyName, int valueType) throws XmlPullParserException, IOException {
        ...
        if (arrayList != null) {
            int count2 = arrayList.size();
            if (count2 > 0) {
                Keyframe firstKeyframe2 = (Keyframe) arrayList.get(0);
                Keyframe lastKeyframe2 = (Keyframe) arrayList.get(count2 - 1);
                float endFraction2 = lastKeyframe2.getFraction();
                if (endFraction2 < 1.0f) {
                    if (endFraction2 < 0.0f) {
                        lastKeyframe2.setFraction(1.0f);
                    } else {
                        arrayList.add(arrayList.size(), createNewKeyframe(lastKeyframe2, 1.0f));
                        count2++;
                    }
                }
                float startFraction2 = firstKeyframe2.getFraction();
                if (startFraction2 != 0.0f) {
                    if (startFraction2 < 0.0f) {
                        firstKeyframe2.setFraction(0.0f);
                    } else {
                        arrayList.add(0, createNewKeyframe(firstKeyframe2, 0.0f));
                        count2++;
                    }
                }
                Keyframe[] keyframeArray2 = new Keyframe[count2];
                arrayList.toArray(keyframeArray2);
                for (int i2 = 0; i2 < count2; i2++) {
                    keyframe = keyframeArray2[i2];
                    if (keyframe.getFraction() < 0.0f) {
                        if (i2 == 0) {
                            keyframe.setFraction(0.0f);
                        } else if (i2 == count2 - 1) {
                            ...
                            int endIndex2 = i2;
                            int j2 = i2 + 1;
                            while (j2 < count2 - 1 && keyframeArray2[j2].getFraction() < 0.0f) {
                                endIndex2 = j2;
                                j2++;
                            }
                            distributeKeyframes(keyframeArray2, keyframeArray2[endIndex2 + 1].getFraction() - keyframeArray2[startIndex2 - 1].getFraction(), startIndex2, endIndex2);
                        }
                    }
                }
                value = PropertyValuesHolder.ofKeyframe(propertyName, keyframeArray2);
                if (valueType == 3) {
                    value.setEvaluator(ArgbEvaluator.getInstance());
                }
            }
