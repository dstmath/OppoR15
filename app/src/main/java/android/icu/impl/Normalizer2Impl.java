        ...
        while (true) {
            ...
            if (src == prevSrc) {
                ...
                if (isJamoVT(norm16) && prevBoundary != prevSrc) {
                    ...
                    if (c < 4519) {
                        ...
                        if (prev < 19) {
                            ...
                            char syllable2 = (char) ((((prev * 21) + (c - 4449)) * 28) + Hangul.HANGUL_BASE);
                            if (src != limit) {
                                char t2 = (char) (s.charAt(src) - 4519);
                                if (t2 < 28) {
                                    src++;
                                    prevBoundary = src;
                                    buffer.setLastChar((char) (syllable2 + t2));
                                }
                            }
                            needToDecompose = true;
                        }
                ...
                if (norm16 >= 65281) {
                    int cc2 = norm16 & 255;
                    if (onlyContiguous) {
                        if ((doCompose ? buffer.getLastCC() : prevCC) == 0 && prevBoundary < prevSrc && getTrailCCFromCompYesAndZeroCC(s, prevBoundary, prevSrc) > cc2) {
                            if (!doCompose) {
                                return false;
                            }
                        }
