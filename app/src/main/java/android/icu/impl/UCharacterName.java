        ...
        while (result <= 32) {
            ...
            if (choice == 0 || choice == 2) {
                ...
                while (count < len) {
                    ...
                    count++;
                    if (b < this.m_tokentable_.length) {
                        char token2 = this.m_tokentable_[b & 255];
                        if (token2 == 65534) {
                            token2 = this.m_tokentable_[(b << 8) | (this.m_groupstring_[index + count] & 255)];
                            count++;
                        }
                        if (token2 == 65535) {
                            nindex2 = nindex + 1;
                            ...
                            nindex2 = UCharacterUtility.compareNullTermByteSubString(name, this.m_tokenstring_, nindex, token2);
                        }
                    } else {
                        nindex2 = nindex + 1;
                        if (name.charAt(nindex) != (b & 255)) {
