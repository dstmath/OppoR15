...
public final class UTS46 extends IDNA {
    ...
    private int processLabel(StringBuilder dest, int labelStart, int labelLength, boolean toASCII, Info info) {
        ...
        if (labelLength != 0) {
            ...
            int i2 = labelStart;
            int limit2 = labelStart + labelLength;
            int oredChars22 = 0;
            boolean disallowNonLDHDot2 = (this.options & 2) != 0;
            do {
                char c3 = labelString.charAt(i2);
                if (c3 > 127) {
                    char oredChars222 = (char) (oredChars222 | c3);
                    if (disallowNonLDHDot2 && isNonASCIIDisallowedSTD3Valid(c3)) {
                        IDNA.addLabelError(info, Error.DISALLOWED);
                        labelString.setCharAt(i2, 65533);
                    } else if (c3 == 65533) {
                        IDNA.addLabelError(info, Error.DISALLOWED);
                    }
                } else if (c3 == '.') {
                    IDNA.addLabelError(info, Error.LABEL_HAS_DOT);
                    labelString.setCharAt(i2, 65533);
                } else if (disallowNonLDHDot2 && asciiData[c3] < (byte) 0) {
                    IDNA.addLabelError(info, Error.DISALLOWED);
                    labelString.setCharAt(i2, 65533);
                }
                i2++;
            } while (i2 < limit2);
            int c22 = labelString.codePointAt(labelStart);
            if ((U_GET_GC_MASK(c22) & U_GC_M_MASK) != 0) {
                IDNA.addLabelError(info, Error.LEADING_COMBINING_MARK);
                labelString.setCharAt(labelStart, 65533);
                if (c22 > DateTimePatternGenerator.MATCH_ALL_FIELDS_LENGTH) {
                    labelString.deleteCharAt(labelStart + 1);
                    labelLength--;
                    if (labelString == dest) {
                        destLabelLength--;
            ...
            if (!IDNA.hasCertainLabelErrors(info, severeErrors)) {
                ...
                if (!((this.options & 8) == 0 || (oredChars222 & 8204) != 8204 || (isLabelOkContextJ(labelString, labelStart, labelLength) ^ 1) == 0)) {
                    IDNA.addLabelError(info, Error.CONTEXTJ);
                }
                if ((this.options & 64) != 0 && oredChars222 >= 183) {
                    checkLabelContextO(labelString, labelStart, labelLength, info);
                }
                if (toASCII) {
                    if (wasPunycode) {
                        ...
                        return destLabelLength;
                    } else if (oredChars222 >= 128) {
                        try {
                            StringBuilder punycode2 = Punycode.encode(labelString.subSequence(labelStart, labelStart + labelLength), null);
                            punycode2.insert(0, "xn--");
                            if (punycode2.length() > 63) {
                                IDNA.addLabelError(info, Error.LABEL_TOO_LONG);
                            }
                            return replaceLabel(dest, destLabelStart, destLabelLength, punycode2, punycode2.length());
                        } catch (Throwable e3) {
