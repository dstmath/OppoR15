    ...
    private String format(int tl, boolean inFuture, int[] counts) {
        ...
        while (i <= last) {
            ...
            while (true) {
                j++;
                if (j >= last || ((1 << j) & mask) != 0) {
                    unit2 = TimeUnit.units[i];
                    count2 = counts[i] - 1;
                    cv2 = this.customs.countVariant;
                } else {
                    skipped = true;
                }
            }
            ...
            skipped |= this.data.appendUnit(unit2, count2, cv2, this.customs.unitVariant, countSep, useDigitPrefix, multiple, i == last, wasSkipped, sb);
            wasSkipped = false;
            if (this.customs.separatorVariant == (byte) 0 || j > last) {
                useDigitPrefix = false;
            } else {
                useDigitPrefix = this.data.appendUnitSeparator(unit2, this.customs.separatorVariant == (byte) 2, i == first, j == last, sb);
            }
            i = j;
        }
        this.data.appendSuffix(tl, td, sb);
