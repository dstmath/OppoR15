                ...
                if (-assertionsDisabled || lowStart == 768) {
                    ...
                    if (-assertionsDisabled || highLimit == 65280) {
                        ...
                        if (lowStart > highLimit) {
                            ...
                            i2 = 1;
                            while (true) {
                                int nextOffset2 = offset2;
                                while (i2 < this.scriptStarts.length - 1) {
                                    int newLeadByte2 = table[i2];
                                    if (newLeadByte2 != 255) {
                                        nextOffset2 = newLeadByte2 - (this.scriptStarts[i2] >> 8);
                                        if (nextOffset2 != offset2) {
                                            break;
                                        }
                                    }
                                    i2++;
                                }
                                if (offset2 != 0 || i2 < this.scriptStarts.length - 1) {
                                    ranges.addElement((this.scriptStarts[i2] << 16) | (DateTimePatternGenerator.MATCH_ALL_FIELDS_LENGTH & offset2));
                                }
                                if (i2 != this.scriptStarts.length - 1) {
                                    offset2 = nextOffset2;
                                    i2++;
