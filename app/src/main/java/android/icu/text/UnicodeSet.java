                ...
                if (!literal) {
                    switch (c) {
                        ...
                        case 123:
                            ...
                            if (op != '-') {
                                add(curString);
                                CharSequence lastString22 = curString;
                            } else {
                                CharSequence charSequence22;
                                if (lastString == null) {
                                    charSequence22 = "";
                                } else {
                                    Object charSequence222 = lastString;
                                }
                                int lastSingle2 = CharSequences.getSingleCodePoint(charSequence222);
                                int curSingle2 = CharSequences.getSingleCodePoint(curString);
                                if (lastSingle2 == Integer.MAX_VALUE || curSingle2 == Integer.MAX_VALUE) {
                                    try {
                                        StringRange.expand(lastString, curString, true, this.strings);
                                    } catch (Exception e22) {
                                        syntaxError(chars, e22.getMessage());
                                    }
                                } else {
                                    add(lastSingle2, curSingle2);
                                }
                                lastString = null;
                                op = 0;
                            }
