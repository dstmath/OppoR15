...
public final class AlphabeticIndex<V> implements Iterable<Bucket<V>> {
    ...
    private BucketList<V> createBucketList() {
        ...
        for (String current : indexCharacters) {
            ...
            i = bucketList.size() - 2;
            while (true) {
                Bucket<V> singleBucket2 = (Bucket) bucketList.get(i);
                if (singleBucket2.labelType != LabelType.NORMAL) {
                    break;
                }
                if (singleBucket2.displayBucket == null) {
                    if ((hasMultiplePrimaryWeights(this.collatorPrimaryOnly, variableTop, singleBucket2.lowerBoundary) ^ 1) != 0) {
                        bucket = new Bucket("", current + "￿", LabelType.NORMAL, null);
                        bucket.displayBucket = singleBucket2;
                        bucketList.add(bucket);
                        hasInvisibleBuckets = true;
                        break;
                    }
