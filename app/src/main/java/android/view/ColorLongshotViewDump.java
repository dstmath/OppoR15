...
public class ColorLongshotViewDump {
    ...
    public void dumpViewRoot(ViewRootImpl viewAncestor, ParcelFileDescriptor fd, List<ColorWindowNode> systemWindows, List<ColorWindowNode> floatWindows) {
        ...
        try {
            ColorLog.d("LongshotDump", "dumpViewRoot : viewAncestor.mView=" + viewAncestor.mView);
    ...
    private boolean isValidScrollNode(View view) {
        boolean z = true;
        ...
        if (view instanceof ScrollView) {
            ...
            return height >= this.mMinScrollHeight || (view.computeVerticalScrollRange() - scrollExtent) * height >= this.mMinScrollDistance * scrollExtent;
            ...
            if (this.mScreenHeight - rect.bottom >= this.mMinListHeight) {
                z = false;
            }
            return z;
        }
    ...
    private void dumpScrollNodes(ViewNode scrollNode, View view, Point minSize, List<View> small, int recursion) {
        if (view != null && (view instanceof ViewGroup) && view.isVisibleToUser()) {
            ...
            if (scrollable) {
                ...
                } else if (isValidScrollNode(view)) {
                ...
                ColorLog.d("LongshotDump", msg.toString());
            }
            if (recursion <= 1) {
                ...
                while (i < childrenCount) {
                    ...
                    dumpScrollNodes(scrollNode, child, minSize, small, recursion);
                    i++;
                }
    ...
    private void dumpViewHierarchy(ColorLongshotDump result, View view) {
        ...
        printTag(tag);
        dumpScrollNodes(null, view, null, null, 0);
        if (this.mScrollNodes.isEmpty()) {
            for (View v : this.mSmallViews) {
                dumpScrollNodes(null, v, null, null, 0);
            }
        }
    ...
    private void selectScrollNodes(List<ViewNode> scrollNodes) {
        if (!scrollNodes.isEmpty()) {
            ...
            selectScrollNodes(((ViewNode) scrollNodes.get(0)).getChildList());
        }
    }
    ...
    private void calcScrollRectForViews(ColorLongshotViewUtils utils, Rect dstRect, Rect srcRect, View view) {
        ...
        while (parent instanceof ViewGroup) {
            ...
            if (group.getChildCount() > 1) {
                ...
                ColorLog.d("LongshotDump", "    nofindCoverRect : rootRect=" + srcRect + ", srcRect=" + this.mTempRect1 + ", group=" + group);
            }
            view = group;
    ...
    private void calcScrollRectForWindow(ColorLongshotViewUtils utils, Rect dstRect, Rect srcRect, ColorWindowNode window) {
        ...
        if (!isInvalidIntersect(this.mTempRect1, srcRect)) {
            String tag = updateCoverRect(utils, dstRect, srcRect, this.mTempRect1) ? "update" : "skip  ";
            StringBuilder msg = new StringBuilder();
            msg.append("    ");
            msg.append(tag);
            msg.append(" : {");
    ...
    private void calcScrollRect(ColorLongshotDump result, ColorLongshotViewUtils utils, ViewNode scrollNode, List<ColorWindowNode> list, List<ColorWindowNode> list2) {
        ColorLog.d("LongshotDump", "==========calcScrollRect====: ");
        if (scrollNode == null) {
            ColorLog.d("LongshotDump", "  calcScrollRect, scrollNode=null");
            return;
        }
        long timeStart = SystemClock.uptimeMillis();
        ...
        for (ViewNode viewNode : scrollNodes) {
            ...
            if (view instanceof ViewGroup) {
                ...
                try {
                    ...
                    ColorLog.e("LongshotDump", "scrollNodesToJson:" + Log.getStackTraceString(e));
                    if (preorderedList != null) {
                        preorderedList.clear();
                    }
                } catch (Throwable th) {
        ...
        if (!windows.isEmpty()) {
            ...
            for (ColorWindowNode window : windows) {
                try {
                    ...
                    ColorLog.e("LongshotDump", "windowNodesToJson:" + Log.getStackTraceString(e));
                }
            }
        }
        ...
        if (!sideRects.isEmpty()) {
            ...
            for (Rect rect : sideRects) {
                try {
                    ...
                    ColorLog.e("LongshotDump", "sideRectsToJson:" + Log.getStackTraceString(e));
                }
            }
        }
        return jsSideArray;
        ...
        if (!floatRects.isEmpty()) {
            ...
            for (Rect rect : floatRects) {
                try {
                    ...
                    ColorLog.e("LongshotDump", "floatRectsToJson:" + Log.getStackTraceString(e));
                }
            }
        }
        return jsFloatArray;
    ...
    private String packJsonNode(ColorLongshotDump result, PrintWriter pw, List<ColorWindowNode> systemWindows, List<ColorWindowNode> floatWindows) {
        ...
        try {
            ...
            ColorLog.e("LongshotDump", "packJsonNode:" + Log.getStackTraceString(e));
        }
