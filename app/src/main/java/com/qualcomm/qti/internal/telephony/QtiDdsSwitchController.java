...
public class QtiDdsSwitchController {
            ...
            switch (msg.what) {
                ...
                case SubsidyLockSettingsObserver.AP_LOCKED /*102*/:
                    ...
                    if (!QtiDdsSwitchController.this.mNotifyCallState) {
                        ...
                        if (QtiDdsSwitchController.this.isFeatureEnabled()) {
                            -wrap0 = QtiDdsSwitchController.this.isCallOnNonDds(phoneId);
                        } else {
                            -wrap0 = false;
                        }
                        qtiDdsSwitchController.mNotifyCallState = -wrap0;
                        QtiDdsSwitchController.this.log("mUserDdsSubId - " + QtiDdsSwitchController.this.mUserDdsSubId);
                    }
                    if (QtiDdsSwitchController.this.mNotifyCallState) {
    ...
    public void updateLplusLStatus(boolean status) {
        ...
        int i;
        if (this.isLplusLSupported && isFeatureEnabled()) {
    ...
    private boolean isCallIdle(int phoneId) {
        return this.mFgCsCalls[phoneId].getState() == State.IDLE && this.mBgCsCalls[phoneId].getState() == State.IDLE && this.mRiCsCalls[phoneId].getState() == State.IDLE && this.mFgImsCalls[phoneId].getState() == State.IDLE && this.mBgImsCalls[phoneId].getState() == State.IDLE && this.mRiImsCalls[phoneId].getState() == State.IDLE;
    }
