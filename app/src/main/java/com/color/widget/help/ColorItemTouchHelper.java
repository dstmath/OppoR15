        ...
        if (selected != this.mSelected || actionState != this.mActionState) {
            ...
            if (this.mSelected != null) {
                ...
                if (prevSelected.itemView.getParent() != null) {
                    ...
                    final ColorItemTouchHelper colorItemTouchHelper = this;
                    ...
                    RecoverAnimation rv = new RecoverAnimation(this, prevSelected, animationType, prevActionState, currentTranslateX, currentTranslateY, targetTranslateX, targetTranslateY) {
                        public void onAnimationEnd(Animator animation) {
                            ...
                            if (!this.mOverridden) {
                                if (swipeDir <= 0) {
                                    colorItemTouchHelper.mCallback.clearView(colorItemTouchHelper.mRecyclerView, viewHolder);
                                } else {
                                    colorItemTouchHelper.mPendingCleanup.add(viewHolder.itemView);
                                    this.mIsPendingCleanup = true;
                                    if (swipeDir > 0) {
                                        colorItemTouchHelper.postDispatchSwipe(this, swipeDir);
                                    }
                                }
                                if (colorItemTouchHelper.mOverdrawChild == viewHolder.itemView) {
                                    colorItemTouchHelper.removeChildDrawingOrderCallbackIfNecessary(viewHolder.itemView);
                                }
                            }
                        }
                    };
