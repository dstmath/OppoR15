...
public class ColorSlideView extends LinearLayout {
    ...
    public void setDeleteEnable(boolean enable) {
        if (this.mCanDelete != enable) {
            ...
            if (enable) {
                this.mItems.add(0, new ColorSlideMenuItem(this.mContext, 201590093, 201852139));
                if (this.mPaint != null) {
    ...
    void drawItemBackground(Canvas canvas) {
        if (this.mItemCount > 0) {
            ...
            if (getLayoutDirection() == 1) {
                canvas.clipRect(0, 0, -getSlideViewScrollX(), h);
            } else {
            ...
            for (int i = 0; i < this.mItemCount; i++) {
                ...
                if (getLayoutDirection() == 1) {
                    tempDrawable.setBounds(position, 0, ((ColorSlideMenuItem) this.mItems.get(i)).getWidth() + position, h);
                    ...
                    tempDrawable.setBounds((this.mHolderWidth - position) - ((ColorSlideMenuItem) this.mItems.get(i)).getWidth(), 0, this.mHolderWidth - position, h);
                    tempDrawable.draw(canvas);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (!this.mSlideEnable) {
            ...
            if (actionMasked == 0 && ((getLayoutDirection() == 1 && event.getX() > ((float) this.mHolderWidth)) || (getLayoutDirection() != 1 && event.getX() < ((float) (getWidth() - this.mHolderWidth))))) {
                return false;
            }
        }
