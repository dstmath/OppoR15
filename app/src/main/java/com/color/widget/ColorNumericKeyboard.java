...
public class ColorNumericKeyboard extends View implements ColorLongshotUnsupported {
    ...
    private final class PatternExploreByTouchHelper extends ExploreByTouchHelper {
        ...
        private Rect getBoundsForVirtualView(int virtualViewId) {
            ...
            } else if (virtualViewId == ColorNumericKeyboard.TEN) {
        ...
        private int getVirtualViewIdForHit(float x, float y) {
            ...
            if (ColorNumericKeyboard.this.mType == 1) {
                if (ColorNumericKeyboard.this.mIsFinishVibility) {
                    if (index == ColorNumericKeyboard.TEN) {
                        index = 9;
                    } else if (index == 9) {
                        index = -1;
                    } else if (index == ColorNumericKeyboard.ELEVEN) {
                        index = ColorNumericKeyboard.TEN;
                    }
                } else if (index == ColorNumericKeyboard.TEN) {
                    index = 9;
        ...
        public int getItemCounts() {
            ...
            return ColorNumericKeyboard.TEN;
        }
        ...
        public CharSequence getItemDescription(int virtualViewId) {
            ...
            if (ColorNumericKeyboard.this.mType == 1) {
                if (virtualViewId == 9) {
                    description = ColorNumericKeyboard.this.mKeyboardNumbers[ColorNumericKeyboard.TEN] + " ";
                } else if (virtualViewId == ColorNumericKeyboard.TEN) {
    ...
    public ColorNumericKeyboard(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mCircleLineWidth = (float) getDimensionOrFraction(a, 5, displayWidth, displayWidth);
        a.recycle();
        ...
        this.mKeyboardLetterTextSize = context.getResources().getDimension(201654402);
    ...
    private void init() {
        ...
        this.mLetterTextPaint.setAlpha(204);
        this.mLetterTextFontMetrics = this.mLetterTextPaint.getFontMetricsInt();
        this.mCirclePaint.setColor(this.mKeyboardNumberTextColor);
        this.mCirclePaint.setAlpha(ENDALPHA);
        ...
        if (AccessibilityManager.getInstance(this.mContext).isTouchExplorationEnabled()) {
            ...
            if (this.mTouchCell != null) {
                ...
                if (this.mType == 1 && this.mSpecialIndex == ELEVEN) {
                    this.mWordTextColor = this.mWordTextPressColor;
                }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.mTouchCell != null) {
            ...
            if (setTouchIndex(this.mTouchCell) != -1) {
    ...
    private void drawCell(Canvas canvas, int column, int row) {
        ...
        if (this.mType != 1) {
            ...
            int top;
            if ((row * 3) + column == 9) {
                canvas.drawCircle(startX, startY, (float) this.mCircleRadius, this.mCirclePaint);
                if (this.mKeyboardLetterStar != null) {
                    left = ((int) startX) - (this.mKeyboardLetterStar.getIntrinsicWidth() / 2);
                    top = ((int) startY) - (this.mKeyboardLetterStar.getIntrinsicHeight() / 2);
            ...
            if ((row * 3) + column == ELEVEN) {
                canvas.drawCircle(startX, startY, (float) this.mCircleRadius, this.mCirclePaint);
                if (this.mKeyboardLetterWellNumber != null) {
                    left = ((int) startX) - (this.mKeyboardLetterWellNumber.getIntrinsicWidth() / 2);
                    top = ((int) startY) - (this.mKeyboardLetterWellNumber.getIntrinsicHeight() / 2);
        ...
        } else if (this.mIsFinishVibility && this.mType == 1 && (row * 3) + column == ELEVEN && this.mSure != null) {
            this.mWordTextPaint.setColor(this.mWordTextColor);
            float textX = startX - (this.mWordTextPaint.measureText(this.mSure) / 2.0f);
            float textY = (startY - ((float) ((this.mWordTextFontMetrics.descent - this.mWordTextFontMetrics.ascent) / 2))) - ((float) this.mWordTextFontMetrics.ascent);
        ...
        if (!TextUtils.isEmpty(cell.cellNumberStr)) {
            ...
            if ((row * 3) + column == TEN) {
                if (TextUtils.isEmpty(cell.cellLettersStr)) {
                    numberStrBaseLine = (startY - ((float) ((this.mNumberTextFontMetrics.descent - this.mNumberTextFontMetrics.ascent) / 2))) - ((float) this.mNumberTextFontMetrics.ascent);
                } else if (cell.cellLettersStr.lastIndexOf(TEN) <= 0) {
        ...
        if (!TextUtils.isEmpty(cell.cellLettersStr)) {
            ...
            int index = cell.cellLettersStr.lastIndexOf(TEN);
