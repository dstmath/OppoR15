...
public class ColorLockPatternView extends View implements ColorAnimation, ColorLongshotUnsupported {
    ...
    public static class Cell extends com.android.internal.widget.LockPatternView.Cell {
        ...
        ShrinkAnimListener shrinkAnimListener = new ShrinkAnimListener();
        int trueColorBlue;
        int trueColorGreen;
        int trueColorRed;
        int wrongColorBlue;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawPath && this.mIsDrag) {
            ...
            for (i = 0; i < count; i++) {
                ...
                } else if (!((!this.mPatternInProgress && this.mPatternDisplayMode != DisplayMode.Correct && this.mPatternDisplayMode != DisplayMode.Animate) || this.mPatternDisplayMode == DisplayMode.FingerprintMatch || this.mPatternDisplayMode == DisplayMode.FingerprintNoMatch)) {
            ...
            if (!this.mIsDrawLadder || this.mPatternDisplayMode == DisplayMode.Wrong) {
    ...
    private void drawLinePath(Cell cell, ArrayList<Float> point, ArrayList<Float> point1, float centerX, float centerY, Path currentPath, int i, boolean anyCircles, Canvas canvas, ArrayList<Cell> pattern, int count) {
        ...
        if (i != 0) {
            ...
            if (cell.zoomInAnim != null && cell.zoomInAnim.isStarted() && i + 1 < count) {
                point1 = calculateStart(cell, getCenterXForColumn(((Cell) pattern.get(i + 1)).column), getCenterYForRow(((Cell) pattern.get(i + 1)).row), centerX, centerY, point1, ringWidth);
    ...
    private void drawLadder(Cell cell, ArrayList<Float> point1, float centerX, float centerY, Path path, int i, boolean anyCircles, Canvas canvas, ArrayList<Cell> pattern, int count) {
        ...
        if (i == 0) {
            if (i + 1 >= count || ((cell.zoomInAnim == null || !cell.zoomInAnim.isStarted()) && (cell.zoomInAnim == null || (cell.zoomInAnim.isRunning() ^ 1) == 0))) {
                ...
                calculateStartPoints(getCenterXForColumn(((Cell) pattern.get(i + 1)).column), getCenterYForRow(((Cell) pattern.get(i + 1)).row), centerX, centerY, halfEndWidth, ringWidth, frontCell, true);
            }
        } else if (i < 1) {
