...
public class DcTracker extends Handler {
                                ...
                                while (true) {
                                    if ((isBusy || now - DcTracker.this.mWifiConnectTimeStamp < 3000) && wait < 60) {
                                        ...
                                        if (defaultApnContext != null) {
                                            switch (AnonymousClass1.-getcom-android-internal-telephony-DctConstants$StateSwitchesValues()[defaultApnContext.getState().ordinal()]) {
                                                case 2:
                                                case 3:
                                                case 5:
                                                case 6:
    ...
    boolean isDataAllowed(ApnContext apnContext, DataConnectionReasons dataConnectionReasons) {
        ...
        try {
            if (OemConstant.isPoliceVersion(this.mPhone) && !OemConstant.isDataAllow(this.mPhone)) {
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    protected void setInitialAttachApn() {
        ...
        if (iaApnSetting != null) {
            ...
            if (firstApnSetting.types.length == 1) {
                if (!("default".equals(initialAttachApnSetting.types[0]) || "emergency".equals(initialAttachApnSetting.types[0]))) {
    ...
    private void onVoiceCallStarted() {
        ...
        SubscriptionManager su = SubscriptionManager.from(this.mPhone.getContext());
        if ((this.mPhone.getSubId() == SubscriptionManager.getDefaultDataSubId()) && (this.mPhone.getServiceStateTracker().isConcurrentVoiceAndDataAllowed() ^ 1) != 0) {
