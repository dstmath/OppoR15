        ...
        if (subscriptionId != -1) {
            ...
            if (callerCanWriteEmbeddedSubscriptions) {
                if ((canManageActiveSubscription(callingPackage) ^ 1) != 0) {
                    ...
                    addResolutionIntent(extrasIntent2, "android.service.euicc.action.RESOLVE_NO_PRIVILEGES", callingPackage, EuiccOperation.forSwitchNoPrivileges(token, subscriptionId, callingPackage));
                    sendResult(callbackIntent, 1, extrasIntent2);
                    Binder.restoreCallingIdentity(token);
                    return;
                }
            }
