...
public abstract class InboundSmsHandler extends StateMachine {
    ...
    private class WaitingState extends State {
        ...
        public void enter() {
            ...
            this.mTracker = null;
            InboundSmsHandler.this.sendMessageDelayed(10, 30000);
        }
    ...
    private boolean processMessagePart(InboundSmsTracker tracker) {
        ...
        if (messageCount == 1) {
            ...
            if (!(block || tracker == null || this.mContext == null)) {
                block = isBlockSmsOem(this.mContext, destPort, tracker.getDisplayAddress());
            }
        } else {
            cursor = null;
            try {
                    ...
                    if (!block) {
                        ...
                        if (!(block || tracker == null)) {
                            if (this.mContext != null) {
                                block = isBlockSmsOem(this.mContext, destPort, cursor.getString(((Integer) PDU_SEQUENCE_PORT_PROJECTION_INDEX_MAPPING.get(Integer.valueOf(9))).intValue()));
                            }
                        }
                    }
                }
                ...
                loge("Can't access multipart SMS database2", ex2);
                if (cursor != null) {
                    cursor.close();
                }
                return false;
    ...
    /* JADX WARNING: inconsistent code. */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    private boolean isBlockSmsOem(Context context, int destPort, String senderNumber) {
        try {
    ...
    public static boolean isDefaultMmsRegion(Context context) {
        ...
        try {
            ...
            Rlog.d(NotificationChannelController.CHANNEL_ID_SMS, "currentRegion = " + currentRegion + ", defaultRegions = " + defaultRegions);
