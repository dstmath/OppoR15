...
public class GsmCdmaPhone extends Phone {
    ...
    private boolean mSwitchPhoneTypeToGsm;
    ...
    public GsmCdmaPhone(Context context, CommandsInterface ci, PhoneNotifier notifier, boolean unitTestMode, int phoneId, int precisePhoneType, TelephonyComponentFactory telephonyComponentFactory) {
        ...
        this.mSwitchPhoneTypeToGsm = false;
    ...
    public Connection dial(String dialString, UUSInfo uusInfo, int videoState, Bundle intentExtras) throws CallStateException {
        if (!OemConstant.isCallOutEnable(this)) {
            ...
            if ((imsUseEnabled && (isUt ^ 1) != 0) || ((isUt && useImsForUt) || useImsForEmergency)) {
                try {
                    if (DBG) {
            ...
            } else if (this.mSST == null || this.mSST.mSS.getState() != 3 || (VideoProfile.isVideo(videoState) ^ 1) == 0 || (isEmergency ^ 1) == 0) {
    ...
    private void switchVoiceRadioTech(int newVoiceRadioTech) {
        if (this.mCT.isOemInEcm() || this.mCT.isOemInEmergencyCall() || !(this.mCT.getState() == PhoneConstants.State.IDLE || 5 == SubscriptionManager.getSimStateForSlotIndex(getPhoneId()))) {
    ...
    public boolean getSwitchPhoneTypeStatus() {
        return this.mSwitchPhoneTypeToGsm;
    }

    public void setSwitchPhoneTypeStatus(boolean status) {
        this.mSwitchPhoneTypeToGsm = status;
    }
