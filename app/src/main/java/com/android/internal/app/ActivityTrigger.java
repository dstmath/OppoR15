...
public class ActivityTrigger {
    ...
    private static final Context mContext = ActivityThread.currentApplication();
    OppoAppScaleHelper mOppoAppScaleHelper = new OppoAppScaleHelper(mContext);
    ...
    public void activityStartTrigger(Intent intent, ActivityInfo acInfo, ApplicationInfo appInfo, boolean IsInFullScreen) {
        ...
        if (IsInFullScreen) {
            ...
            if (appInfo.packageName != null) {
                float scale = this.mOppoAppScaleHelper.GetScale(appInfo.packageName);
                if (scale > 0.0f && scale != 1.0f) {
                    appInfo.setAppScale(scale);
                    return;
                }
                return;
            }
