    ...
    void populate(int newCurrentItem) {
        ...
        if (this.mAdapter == null) {
            ...
            if (curItem != null) {
                ...
                float extraWidthLeft2 = 0.0f;
                int itemIndex2 = curIndex - 1;
                ii = itemIndex2 >= 0 ? (ItemInfo) this.mItems.get(itemIndex2) : null;
                int clientWidth2 = getPaddedWidth();
                if (clientWidth2 <= 0) {
                    leftWidthNeeded2 = 0.0f;
                } else {
                    leftWidthNeeded2 = (2.0f - curItem.widthFactor) + (((float) getPaddingLeft()) / ((float) clientWidth2));
                }
                int pos2 = this.mCurItem - 1;
                while (pos2 >= 0) {
                    if (extraWidthLeft2 < leftWidthNeeded2 || pos2 >= startPos) {
                        if (ii == null || pos2 != ii.position) {
                            extraWidthLeft2 += addNewItem(pos2, itemIndex2 + 1).widthFactor;
                            curIndex++;
                            ii = itemIndex2 >= 0 ? (ItemInfo) this.mItems.get(itemIndex2) : null;
                        } else {
                            extraWidthLeft2 += ii.widthFactor;
                            itemIndex2--;
                            ii = itemIndex2 >= 0 ? (ItemInfo) this.mItems.get(itemIndex2) : null;
                        }
                    } else if (ii == null) {
                        ...
                        if (pos2 == ii.position && (ii.scrolling ^ 1) != 0) {
                            this.mItems.remove(itemIndex2);
                            this.mAdapter.destroyItem((ViewGroup) this, pos2, ii.object);
                            itemIndex2--;
                            curIndex--;
                            ii = itemIndex2 >= 0 ? (ItemInfo) this.mItems.get(itemIndex2) : null;
                        }
                ...
                itemIndex2 = curIndex + 1;
                if (extraWidthRight2 < 2.0f) {
                    float rightWidthNeeded2;
                    ii = itemIndex2 < this.mItems.size() ? (ItemInfo) this.mItems.get(itemIndex2) : null;
                    if (clientWidth2 <= 0) {
                        rightWidthNeeded2 = 0.0f;
                    } else {
                        rightWidthNeeded2 = (((float) getPaddingRight()) / ((float) clientWidth2)) + 2.0f;
                    }
                    pos2 = this.mCurItem + 1;
                    while (pos2 < N) {
                        if (extraWidthRight2 < rightWidthNeeded2 || pos2 <= endPos) {
                            if (ii == null || pos2 != ii.position) {
                                itemIndex2++;
                                extraWidthRight2 += addNewItem(pos2, itemIndex2).widthFactor;
                                ii = itemIndex2 < this.mItems.size() ? (ItemInfo) this.mItems.get(itemIndex2) : null;
                            } else {
                                extraWidthRight2 += ii.widthFactor;
                                itemIndex2++;
                                ii = itemIndex2 < this.mItems.size() ? (ItemInfo) this.mItems.get(itemIndex2) : null;
                            }
                        } else if (ii == null) {
            ...
            for (i = 0; i < childCount; i++) {
                ...
                lp2.childIndex = i;
                if (!lp2.isDecor && lp2.widthFactor == 0.0f) {
                    ii = infoForChild(child);
                    if (ii != null) {
                        lp2.widthFactor = ii.widthFactor;
                        lp2.position = ii.position;
                    }
                }
            }
            sortChildDrawingOrder();
            if (hasFocus()) {
                View currentFocused2 = findFocus();
                ii = currentFocused2 != null ? infoForAnyChild(currentFocused2) : null;
                if (ii == null || ii.position != this.mCurItem) {
                    for (i = 0; i < getChildCount(); i++) {
                        child = getChildAt(i);
                        ii = infoForChild(child);
                        if (ii != null && ii.position == this.mCurItem) {
                            Rect focusRect2;
                            if (currentFocused2 == null) {
                                focusRect2 = null;
                            } else {
                                focusRect2 = this.mTempRect;
                                currentFocused2.getFocusedRect(this.mTempRect);
                                offsetDescendantRectToMyCoords(currentFocused2, this.mTempRect);
                                offsetRectIntoDescendantCoords(child, this.mTempRect);
                            }
                            if (child.requestFocus(focusDirection, focusRect2)) {
                                break;
                            }
                        }
                    }
