...
final class TaskRecord extends ConfigurationContainer implements TaskWindowContainerListener {
    ...
    private boolean forceInvibleInPreload = false;
    ...
    boolean reparent(int preferredStackId, int position, int moveStackMode, boolean animate, boolean deferResume, boolean schedulePictureInPictureModeChange, String reason) {
        ...
        if (sourceStackId == 7) {
            if (this.mStack != null) {
                r = this.mStack.topActivity();
                if (r != null) {
                    this.mStack.makeInvisible(r);
                }
            }
        ...
        try {
            ...
            if (!(sourceStack == null || sourceStack.mStackId != 7 || sourceStack.mStackId == toStack.mStackId || this.mActivities == null || sourceStack.mLRUActivities == null)) {
                for (ActivityRecord ar : this.mActivities) {
                    if (ar != null) {
                        sourceStack.mLRUActivities.remove(ar);
                    }
                }
            }
            ...
            int i;
            ActivityRecord tmp;
            if (!deferResume && sourceStackId != 7) {
    ...
    boolean updateOverrideConfiguration(Rect bounds, Rect insetBounds) {
        ...
        if (this.mFullscreen) {
            ...
            computeOverrideConfiguration(newConfig, rect, insetBounds, z, z2);
        }
        onOverrideConfigurationChanged(newConfig);
        if (this.mFullscreen != oldFullscreen) {
            this.mService.mStackSupervisor.scheduleUpdateMultiWindowMode(this);
        }
        if (this.mTmpConfig == null || this.mTmpConfig == Configuration.EMPTY || (newConfig != null && newConfig.appBounds != null)) {
    ...
    void computeOverrideConfiguration(Configuration config, Rect bounds, Rect insetBounds, boolean overrideWidth, boolean overrideHeight) {
        ...
        if (this.mStack != null) {
            ...
            if (this.mStack != null && this.mStack.getStackId() == 7) {
                Configuration serviceConfig = this.mService.getConfiguration();
                config.screenWidthDp = (int) (((float) this.mTmpStableBounds.width()) / density);
                config.screenHeightDp = (int) (((float) this.mTmpStableBounds.height()) / density);
                config.densityDpi = serviceConfig.densityDpi;
                config.smallestScreenWidthDp = serviceConfig.smallestScreenWidthDp;
            }
    ...
    private boolean isPreloadTask() {
        if (this.mStack != null && this.mStack.mStackId == 7) {
            return true;
        }
        return false;
    }

    public boolean getForceInvibleInPreload() {
        return this.forceInvibleInPreload;
    }

    public void setForceInvibleInPreload(boolean invible) {
        this.forceInvibleInPreload = invible;
    }
}
