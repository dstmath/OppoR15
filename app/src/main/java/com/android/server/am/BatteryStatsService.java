...
public final class BatteryStatsService extends Stub implements LowPowerModeListener, PlatformIdleStateCallback {
    ...
    /* synthetic */ void lambda$-com_android_server_am_BatteryStatsService_33725(int plugType, int status, int health, int level, int temp, int volt, int chargeUAh, int chargeFullUAh) {
        synchronized (this.mStats) {
    ...
    /* synthetic */ void lambda$-com_android_server_am_BatteryStatsService_35112(int status, int health, int plugType, int level, int temp, int volt, int chargeUAh, int chargeFullUAh) {
        synchronized (this.mStats) {
            this.mStats.setBatteryStateLocked(this.mContext, status, health, plugType, level, temp, volt, chargeUAh, chargeFullUAh);
        }
    }
                        ...
                        synchronized (this.mStats.mCheckinFile) {
                            if (this.mStats.mCheckinFile.exists()) {
                                try {
                                    ...
                                    Slog.w(TAG, "Failure reading checkin file " + this.mStats.mCheckinFile.getBaseFile(), e22);
                                }
                            }
                        }
                    } else {
