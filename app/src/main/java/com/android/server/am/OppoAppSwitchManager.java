...
public class OppoAppSwitchManager {
    ...
    public void handleActivitySwitch(Context context, ActivityRecord prev, ActivityRecord next, boolean dontWaitForPause, ComponentName lastCpn) {
        ...
        if (prev == null) {
            ...
            notifyActivityChanged(context, next.packageName, lastPkg, prevActivity, nextActivity, false, isNextMultiApp, false, isNextForFreeForm);
            ...
            if (prev.packageName != null) {
                ...
                if (!next.packageName.equals(lastPkg) || (OppoListManager.getInstance().isRedundentActivity(prevActivity) ^ 1) == 0 || (OppoListManager.getInstance().isRedundentActivity(nextActivity) ^ 1) == 0 || (isFreeFormMMAppBrandUI ^ 1) == 0) {
                    ...
                    notifyActivityChanged(context, next.packageName, lastPkg, prevActivity, nextActivity, isPreMultiApp, isNextMultiApp, isPreForFreeForm, isNextForFreeForm);
                } else {
                    if (this.mDynamicDebug) {
    ...
    private void notifyActivityChanged(Context context, String nextPkgName, String prePkgName, String prevActivity, String nextActivity, boolean isPreMultiApp, boolean isNextMultiApp, boolean isPreForFreeForm, boolean isNextForFreeForm) {
        sendAppSwitchBroadcast(context, nextPkgName, prePkgName, prevActivity, nextActivity);
    ...
    public void sendAppSwitchBroadcast(Context context, String nextPkgName, String prePkgName, String prevActivity, String nextActivity) {
        if (nextPkgName == null) {
            if (sDebugfDetail) {
