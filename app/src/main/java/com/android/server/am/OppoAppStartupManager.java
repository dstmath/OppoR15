...
public class OppoAppStartupManager {
    ...
    private static final String AUTHORITY = "com.coloros.oppoguardelf.abnormalmanager.provider";
    private static final Uri AUTHORITY_URI = Uri.parse("content://com.coloros.oppoguardelf.abnormalmanager.provider");
    private static final String CALL_ARG_CRASH = "crash";
    private static final String CALL_ARG_STARTUP = "startup";
    private static final String CALL_FRAMEWORK = "framework";
    ...
    private static final Uri STATUP_MONITOR_PROVIDER_URI = Uri.withAppendedPath(AUTHORITY_URI, TABLE);
    ...
    private class AppAbnormalMonitor implements Runnable {
        ...
        public void run() {
            OppoAppStartupManager.this.onAppAbnormalMonitor(this.mPkg, this.mExceptionClass, this.mExceptionMsg, this.mExceptionTrace, this.mType);
        }
    }
    ...
    private IContentProvider getIContentProvider() {
        if (this.mIContentProvider == null) {
            try {
                this.mIContentProvider = this.mAms.mContext.getContentResolver().acquireUnstableProvider(STATUP_MONITOR_PROVIDER_URI);
            } catch (Exception e) {
                Log.e("OppoAppStartupManager", "getIContentProvider failed!!");
                this.mIContentProvider = null;
            }
        }
    ...
    private void onAppAbnormalMonitor(String packageName, String exceptionClass, String exceptionMsg, String exceptionTrace, String type) {
        if (this.mAms != null && this.mAms.mContext != null && getIContentProvider() != null) {
            try {
                Bundle bundle = new Bundle();
                if (CALL_ARG_STARTUP.equals(type)) {
                    bundle.putString("pkg", packageName);
                } else if (CALL_ARG_CRASH.equals(type)) {
                    bundle.putString("pkg", packageName);
                    bundle.putString(AudioService.CONNECT_INTENT_KEY_DEVICE_CLASS, exceptionClass);
                    bundle.putString("msg", exceptionMsg);
                    bundle.putString("trace", exceptionTrace);
                }
                Bundle call = this.mIContentProvider.call(CALL_FRAMEWORK, CALL_FRAMEWORK, type, bundle);
            } catch (Exception e) {
                Log.e("OppoAppStartupManager", "ContentProvider onUpdateStartupMonitor failed!!");
                this.mIContentProvider = null;
            }
        }
    }
