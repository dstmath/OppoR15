...
final class ActivityRecord extends ConfigurationContainer implements AppWindowContainerListener {
    ...
    void completeResumeLocked() {
        ...
        if (!OifaceUtil.isEnable() || this.realActivity == null || this.app == null) {
            Slog.i(TAG, "OifaceUtil cann't get currentPackage");
        } else {
            OifaceUtil.getInstance().currentPackage(this.realActivity.getPackageName(), this.app.uid, this.app.pid);
        }
        ComponentName componentName = this.realActivity;
        if (componentName != null && this.service.mNs != null && this.app != null) {
            ContentValues contentValues = new ContentValues();
            contentValues.put("pkgname", componentName.getPackageName());
            contentValues.put(OppoAppStartupManager.TYPE_ACTIVITY, componentName.getShortClassName());
            contentValues.put("date", Long.valueOf(System.currentTimeMillis()));
            contentValues.put("uid", Integer.valueOf(this.app.uid));
            contentValues.put("pid", Integer.valueOf(this.app.pid));
            this.service.mNs.publishEvent(1, contentValues);
        }
    }
    ...
    void setRequestedOrientation(int requestedOrientation) {
        if (this.task == null || this.task.mStack == null || this.task.mStack.mStackId != 7) {
        ...
        boolean isNeedUpdateConfig = false;
        boolean isLandscapeApp = false;
        if (requestedOrientation == 0 || requestedOrientation == 6) {
            isLandscapeApp = true;
        }
        Configuration prevConfig = this.task.getOverrideConfiguration() != Configuration.EMPTY ? this.task.getOverrideConfiguration() : this.service.getConfiguration();
        if ((prevConfig.screenHeightDp > prevConfig.screenWidthDp && isLandscapeApp) || (prevConfig.screenHeightDp < prevConfig.screenWidthDp && (isLandscapeApp ^ 1) != 0)) {
            isNeedUpdateConfig = true;
        }
        if (isNeedUpdateConfig) {
            this.task.updateOverrideConfiguration(this.service.mActivityStarter.getPreloadLaunchBounds(this, isLandscapeApp));
            getTask().getWindowContainerController().resize(getTask().mBounds, getTask().getOverrideConfiguration(), true, false);
            this.mWindowContainerController.setOrientationOnly(requestedOrientation);
        }
    }
    ...
    boolean ensureActivityConfigurationLocked(int globalChanges, boolean preserveWindow) {
        ...
        if (stack.mConfigWillChange) {
            ...
            if (this.mTmpConfig.diff(getConfiguration()) == 1024 && this.mTmpConfig.screenWidthDp == getConfiguration().screenWidthDp && this.mTmpConfig.screenHeightDp == getConfiguration().screenHeightDp && this.mTmpConfig.appBounds != getConfiguration().appBounds) {
                this.mTmpConfig.appBounds = getConfiguration().appBounds;
            }
            if (!getConfiguration().equals(this.mTmpConfig) || (this.forceNewConfig ^ 1) == 0 || (displayChanged ^ 1) == 0 || (this.mShouldRelaunch ^ 1) == 0) {
                ...
                if (!(this.task == null || (this.task.mFullscreen ^ 1) == 0 || this.info == null)) {
    ...
    private boolean shouldRelaunchLocked(int changes, Configuration changesConfig) {
        boolean z = false;
        if ((134217728 & changes) == 0 || !OppoThemeUtil.THEME_CHANGED_IGNORE_PKGS.contains(this.processName)) {
        ...
        Log.i(TAG, "shouldRelaunchLocked. Not to relaunch Activitys of ignore apps when skin changed. processName = " + this.processName + " , packageName = " + this.packageName);
        return false;
    }
    ...
    static ActivityRecord restoreFromXml(XmlPullParser in, ActivityStackSupervisor stackSupervisor) throws IOException, XmlPullParserException {
        ...
        if (intent != null) {
            ActivityManagerService service2 = stackSupervisor.mService;
            ActivityInfo aInfo2 = stackSupervisor.resolveActivity(intent, resolvedType, 0, null, userId);
            if (aInfo2 == null) {
                throw new XmlPullParserException("restoreActivity resolver error. Intent=" + intent + " resolvedType=" + resolvedType);
            }
            ActivityRecord r2 = new ActivityRecord(service2, null, 0, launchedFromUid, launchedFromPackage, intent, resolvedType, aInfo2, service2.getConfiguration(), null, null, 0, componentSpecified, false, stackSupervisor, null, null);
            r2.persistentState = persistentState;
