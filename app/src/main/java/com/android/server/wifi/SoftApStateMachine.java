...
public class SoftApStateMachine extends StateMachine {
    ...
    class SoftApState extends State {
        ...
        public void enter() {
            ...
            SoftApStateMachine.this.mSoftApManager = SoftApStateMachine.this.mWifiInjector.makeSoftApManager(SoftApStateMachine.this.mNwService, new SoftApListener(), iApInterface, config.getWifiConfiguration());
    ...
    private void setWifiApState(int wifiApState, int reason, String ifaceName, int mode) {
        ...
        Intent intent;
        if ((!sharingManager.isWifiSharingTethering() || (sharingManager.isWifiApEnabledState() ^ 1) == 0) && !((wifiApState == 10 || wifiApState == 11) && sharingManager.isWifiSharingEnabledState())) {
            Log.e(TAG, "SoftApStateMachine notificaiton for Ap " + wifiApState);
        ...
        Log.e(TAG, "SoftApStateMachine notificaiton for Sharing" + wifiApState);
        intent = new Intent("oppo.intent.action.wifi.WIFI_SHARING_STATE_CHANGED");
        intent.addFlags(67108864);
        intent.putExtra("wifi_state", sharingManager.transformApState2SharingState(wifiApState));
        intent.putExtra("previous_wifi_state", sharingManager.transformApState2SharingState(previousWifiApState));
        if (wifiApState == 14) {
            intent.putExtra("wifi_ap_error_code", reason);
        }
        if (ifaceName == null) {
        ...
        intent.putExtra("wifi_ap_mode", mode);
        this.mContext.sendStickyBroadcastAsUser(intent, UserHandle.ALL);
        sharingManager.updateWifiSharingPersistState(sharingManager.transformApState2SharingState(wifiApState));
    }
