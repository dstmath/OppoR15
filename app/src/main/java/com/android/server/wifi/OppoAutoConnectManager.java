...
public class OppoAutoConnectManager {
    ...
    private OppoManuConnectState mOppoManuConnectState = new OppoManuConnectState();
    ...
    public class OppoManuConnectState {
        ...
        private int mNetId;
        private int mState;
        private HashMap<Integer, String> mStateStrHashMap;
        private int mType;
        private HashMap<Integer, String> mTypeStrHashMap;

        public int getState() {
            OppoAutoConnectManager.this.logd("get manu connect State:" + ((String) this.mStateStrHashMap.get(Integer.valueOf(this.mState))));
            return this.mState;
        }
        ...
        public int getType() {
            OppoAutoConnectManager.this.logd("get manu connect type:" + ((String) this.mTypeStrHashMap.get(Integer.valueOf(this.mType))));
            return this.mType;
        }
        ...
        public int getNetId() {
            OppoAutoConnectManager.this.logd("get manu connect NetId:" + this.mNetId);
            return this.mNetId;
        }
        ...
        public String toString() {
            return "manu connect: netId=" + this.mNetId + ",type=[" + this.mType + "]" + ((String) this.mTypeStrHashMap.get(Integer.valueOf(this.mType))) + ",state=[" + this.mState + "]" + ((String) this.mStateStrHashMap.get(Integer.valueOf(this.mState)));
        }
        ...
        public OppoManuConnectState() {
            ...
            this.mType = 0;
            this.mNetId = 0;
            this.mStateStrHashMap = new HashMap();
            this.mTypeStrHashMap = new HashMap();
            this.mState = -1;
            this.mType = -1;
            this.mNetId = -1;
    ...
    private long getKeepDisabledUntil(OppoAutoConnectConfiguration oppoAutoConnConf) {
        ...
        switch (oppoAutoConnConf.mDisabledReason) {
            case 2:
                if (continuousDisabledCount >= assocRejectStepCount) {
                    ...
                    if (disabledLevel > rssiGood) {
                        ...
                        keepDisabledUntilAdd = (long) ((assocRejectMultipleBase * integerMultiple) * 1000);
                    }
            ...
            case 3:
                if (continuousDisabledCount >= authFailureStepCount) {
                    ...
                    if (disabledLevel > rssiGood) {
                        ...
                        keepDisabledUntilAdd = (long) ((authFailureMultipleBase * integerMultiple) * 1000);
                    }
            ...
            case 4:
                if (continuousDisabledCount >= dhcpFailureStepCount) {
                    ...
                    if (disabledLevel > rssiGood) {
                        ...
                        keepDisabledUntilAdd = (long) ((dhcpFailureMultipleBase * integerMultiple) * 1000);
                    }
            ...
            case 12:
                if (continuousDisabledCount >= wrongKeyStepCount) {
                    ...
                    if (disabledLevel > rssiGood) {
                        ...
                        keepDisabledUntilAdd = (long) ((wrongKeyMultipleBase * integerMultiple) * 1000);
                    }
            ...
            case ChannelHelper.SCAN_PERIOD_PER_CHANNEL_MS /*200*/:
                if (continuousDisabledCount >= continuousDebounceDisconnectStepCount) {
                    keepDisabledUntilAdd = (long) ((continuousDebounceDisconnectStepTime * (continuousDisabledCount / continuousDebounceDisconnectStepCount)) * 1000);
                    break;
                }
                baseTime = oppoAutoConnConf.mDisabledUntilTime;
                break;
                ...
                case 0:
                    ...
                    if (this.mLastConnectedConfiguration != null && this.mLastConnectedConfiguration.mConfigKey != null) {
                        if (this.mLastConnectedConfiguration.mConfigKey.equals(configKey)) {
                            ...
                            if (this.mLastConnectedConfiguration.mConituousDebounceDisconnectCount >= continuousDebounceDisconnectStepCount && this.mLastConnectedConfiguration.mConituousDebounceDisconnectCount % continuousDebounceDisconnectStepCount == 0) {
                ...
                case 2:
                    ...
                    if (!(this.mOppoManuConnectState.getState() != 1 || this.mOppoManuConnectState.getNetId() == networkId || this.mOppoManuConnectState.getNetId() == -1)) {
                        ...
                        if (sWifiNetworkStateTraker != null) {
                            sWifiNetworkStateTraker.setManualConnect(false, false, 1000);
                        }
                    }
    ...
    boolean hasValidSingtelOperatorSimCard() {
        ...
        if (getValidSingtelOpratorSimCardNum() > 0) {
            z = true;
        }
        return z;
    }
        ...
        if (wcList == null || wcList.size() <= 0) {
            ...
            if (candicatList.size() == 1) {
                ...
                for (WifiConfiguration wcc : candicatList) {
                    if (candicator == null) {
                        candicator = wcc;
                    } else if (wcc.priority > candicator.priority) {
                        candicator = wcc;
                    }
                }
            }
            if (candicator == null) {
                ...
                return null;
            } else if (ColorOSTelephonyManager.getDefault(sContext) == null) {
                logd("ColorOSTelephonyManager is null!!");
                return null;
            } else {
                int simSlot = candicator.SIMNum;
                int targetSimCard = simSlot;
                int simCardIndex = getValidSingtelOpratorSimCardNum();
                if (simCardIndex == 0) {
                    logd("no sim card!!");
                    return null;
                }
                if (1 == simCardIndex || 2 == simCardIndex) {
                    ...
                    String configKey = candicator.configKey();
                    if (configKey == null) {
                        logd("configKey is null!!");
                        return null;
                    } else if (getAutoConnectConfiguration(configKey) != null) {
                ...
                if (sWifiConfigManager != null) {
                    sWifiConfigManager.addOrUpdateNetwork(candicator, 1000);
                }
                return candicator;
            }
        } else {
            logd("has no valid singtel simcard or not in singtel region!!");
            return null;
        }
    }
    ...
    private WifiConfiguration pickValidWifiConfiguration() {
        ...
        if (savedNetworksList == null || savedNetworksList.size() <= 0) {
            ...
            WifiConfiguration selectedConf = pickSingtelWifiConfiguration(savedAndEnaledAndScannedNetworkList);
            if (selectedConf != null) {
                logd("pick a Singtel WifiConfiguration:" + selectedConf.configKey());
                return selectedConf;
            }
            if (this.mWifiAssistantEnabled) {
                if ((this.mOppoManuConnectState.getState() == -1 || this.mOppoManuConnectState.getState() == 2) && sWifiStateMachine != null && sWifiStateMachine.isSupplicantStateDisconnected() && sWifiNetworkStateTraker != null && sWifiNetworkStateTraker.getManualConnect()) {
        ...
        if (!checkInterfaceNull()) {
            ...
            if (selectedConf != null) {
                ...
                if (this.mWifiAssistantEnabled) {
                    ...
                    if (isSingtelOperatorAp(selectedConf.networkId) && (isSingtelOperatorAp(this.mConnectingNetId) ^ 1) != 0) {
    ...
    private boolean isWapiPskAp(WifiConfiguration wc) {
        if (wc != null && wc.allowedKeyManagement.get(10)) {
            return true;
        }
        return false;
    }
    ...
    private boolean isWapiCertAp(WifiConfiguration wc) {
        if (wc != null && wc.allowedKeyManagement.get(11)) {
            return true;
        }
        return false;
    }
    ...
    private int getContinuousDebounceDisconnectIntervalTime() {
        ...
        return sWifiRomUpdateHelper.getIntegerValue("OPPO_AUTO_CONNECT_CONTINUOUS_DEBOUNCE_DISCONNECT_INTERVAL_TIME", Integer.valueOf(1000)).intValue();
    }
