...
import android.net.wifi.OppoWifiDevice;
...
public class OppoHotspotClientInfo {
    ...
    private HashMap<String, OppoWifiDevice> mConnectedDeviceMap = new HashMap();
    private HashMap<String, HotspotClient> mHtsptClientMap = new HashMap();
    private HashMap<String, OppoWifiDevice> mL2ConnectedDeviceMap = new HashMap();

    private static class DnsmasqThread extends Thread {
        private OppoWifiDevice mDevice;
        private int mInterval;
        private int mMaxTimes;
        private final OppoHotspotClientInfo mOppoHotspotClientInformation;

        public DnsmasqThread(OppoHotspotClientInfo clientInfo, OppoWifiDevice device, int interval, int maxTimes) {
            super("SoftAp");
        ...
        public void run() {
            ...
            OppoWifiDevice other = (OppoWifiDevice) this.mOppoHotspotClientInformation.mL2ConnectedDeviceMap.get(this.mDevice.deviceAddress);
    ...
    public List<OppoWifiDevice> getConnectedStations() {
        List<OppoWifiDevice> getConnectedStationsList = new ArrayList();
        if (mContext.getResources().getBoolean(17957022)) {
            for (String key : this.mConnectedDeviceMap.keySet()) {
                OppoWifiDevice device = (OppoWifiDevice) this.mConnectedDeviceMap.get(key);
    ...
    private boolean readDeviceInfoFromDnsmasq(OppoWifiDevice device) {
        IOException ex;
        Throwable th;
        boolean result = false;
    ...
    private void interfaceMessageRecevied(String message, boolean isConnected) {
        if (mContext.getResources().getBoolean(17957022)) {
            ...
            OppoWifiDevice device = new OppoWifiDevice(message, isConnected);
            if (device.deviceState == 1) {
                this.mL2ConnectedDeviceMap.put(device.deviceAddress, device);
                Log.d(TAG, "device: connected");
                OppoWifiSharingManager oppoSharingManager = WifiInjector.getInstance().getOppoWifiSharingManager();
                if (oppoSharingManager.isWifiSharingTethering()) {
                    oppoSharingManager.statsStaConnected();
                }
                if (readDeviceInfoFromDnsmasq(device)) {
                    ...
                    new DnsmasqThread(this, device, DNSMASQ_POLLING_INTERVAL, 10).start();
                }
