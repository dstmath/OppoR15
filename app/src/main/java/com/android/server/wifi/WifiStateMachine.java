...
public class WifiStateMachine extends StateMachine implements WifiRssiEventHandler, FilterController {
    ...
    private static boolean sManuConnect = false;
    private static WifiConfiguration sManuConnectConfiguration = null;
    private static int sManuConnectNetId = -1;
    ...
    private static boolean sStartWps = false;
    ...
    private final DisplayListener mDisplayListener = new DisplayListener() {
        ...
        public void onDisplayChanged(int displayId) {
            WifiStateMachine.this.mRotation = WifiStateMachine.this.mDisplay.getRotation();
            boolean shouldEnableAutoSwitch = false;
            WifiStateMachine.this.logd("wifi rt:" + WifiStateMachine.this.mRotation);
            if (WifiStateMachine.this.mRotation == 0) {
                shouldEnableAutoSwitch = false;
            } else if (WifiStateMachine.this.mRotation == 1) {
                shouldEnableAutoSwitch = false;
            } else if (WifiStateMachine.this.mRotation == 2) {
                shouldEnableAutoSwitch = false;
            } else if (WifiStateMachine.this.mRotation == 3 && !WifiStateMachine.this.mAutoSwitch && (WifiStateMachine.this.mWifiNetworkStateTraker.getAutoSwitch() ^ 1) != 0 && WifiStateMachine.this.mWifiInfo.is5GHz() && WifiStateMachine.this.getRomUpdateBooleanValue("OPPO_WIFI_ASSISTANT_RIGHT_ROTATION", Boolean.valueOf(true)).booleanValue()) {
                String gameMode = SystemProperties.get("debug.gamemode.value", "0");
                WifiStateMachine.this.logd("gm=" + gameMode + ", us= " + WifiStateMachine.this.mUserSelectSwith);
                if (WifiStateMachine.this.mUserSelectSwith && gameMode.equals("1")) {
                    shouldEnableAutoSwitch = true;
                }
            }
            if (shouldEnableAutoSwitch) {
                WifiStateMachine.this.mWifiNetworkStateTraker.setAutoSwitch(true);
            } else if (!WifiStateMachine.this.mAutoSwitch && WifiStateMachine.this.mWifiNetworkStateTraker.getAutoSwitch()) {
    ...
    private boolean mUserSelectSwith;
    ...
    private OppoWifiConfigRestore mWifiConfigRestore = new OppoWifiConfigRestore();
            ...
            switch (message.what) {
                case WifiStateMachine.CMD_ENABLE_NETWORK /*131126*/:
                    ...
                    if (disableOthers && enableNetId != -1 && enableNetId == curNetId) {
                        ...
                        WifiStateMachine.sNetId = enableNetId;
                ...
                case 151553:
                    ...
                    if (ignoreDisconnect) {
                        ...
                        WifiStateMachine.sNetId = conId;
                        WifiStateMachine.this.sendConnectedState();
                        WifiStateMachine.this.sendConnectModeChangeBroadcast(true);
                        WifiStateMachine.this.transitionTo(WifiStateMachine.this.mConnectedState);
                        break;
                    }
                    return false;
    ...
    class ConnectModeState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            int reasonCode;
            WifiConnectionStatistics -get93;
            ...
            switch (message.what) {
                case WifiStateMachine.CMD_REMOVE_NETWORK /*131125*/:
                    ...
                    if (WifiStateMachine.sNetId == message.arg1 && WifiStateMachine.sNetId != -1 && WifiStateMachine.sManualConnect) {
                ...
                case WifiStateMachine.CMD_ENABLE_NETWORK /*131126*/:
                    ...
                    if (ok) {
                        if (message.arg2 == 1) {
                            ...
                            WifiStateMachine.sManualConnect = true;
                            WifiStateMachine.sNetId = message.arg1;
                            WifiStateMachine.this.mOppoWifiConnectionAlert.sendEnableNetworkEvent(message.arg1);
                        }
                        if (message.arg2 == 1) {
                            if (WifiStateMachine.this.mWifiNetworkAvailable != null) {
                                WifiStateMachine.this.mWifiNetworkAvailable.setManualConnect(true);
                            } else if (WifiStateMachine.this.mWifiNetworkStateTraker != null) {
                                WifiStateMachine.this.mWifiNetworkStateTraker.setManualConnect(true, false, message.sendingUid);
                ...
                case WifiMonitor.FILS_NETWORK_CONNECTION_EVENT /*147519*/:
                    ...
                    if (WifiStateMachine.sManualConnect && WifiStateMachine.sNetId == WifiStateMachine.this.mLastNetworkId && current != null) {
                    ...
                    if (!WifiStateMachine.sManualConnect) {
                        if (!WifiStateMachine.sConnection) {
                            if (WifiStateMachine.this.mVerboseLoggingEnabled) {
                                Log.d(WifiStateMachine.TAG, "sendConnectModeChangeBroadcast(false) connected id is " + message.arg1);
                            }
                            WifiStateMachine.this.sendConnectModeChangeBroadcast(false);
                            break;
                        }
                        if (WifiStateMachine.this.mVerboseLoggingEnabled) {
                            Log.d(WifiStateMachine.TAG, "Connect to wps AP sendConnectModeChangeBroadcast(true), nNetId is " + message.arg1);
                        }
                        WifiStateMachine.this.sendConnectModeChangeBroadcast(true);
                        break;
                    }
                    if (WifiStateMachine.sNetId != WifiStateMachine.this.mLastNetworkId) {
                        if (WifiStateMachine.this.mVerboseLoggingEnabled) {
                            Log.d(WifiStateMachine.TAG, "sendConnectModeChangeBroadcast(false), sNetId is " + WifiStateMachine.sNetId + " connected id is " + message.arg1);
                        }
                        WifiStateMachine.this.sendConnectModeChangeBroadcast(false);
                        break;
                    }
                    if (WifiStateMachine.this.mVerboseLoggingEnabled) {
                        Log.d(WifiStateMachine.TAG, "sendConnectModeChangeBroadcast(true), sNetId is " + WifiStateMachine.sNetId);
                    }
                    WifiStateMachine.this.sendConnectModeChangeBroadcast(true);
                    WifiStateMachine.sManualConnect = false;
                    break;
                case WifiMonitor.NETWORK_DISCONNECTION_EVENT /*147460*/:
                    if (WifiStateMachine.this.mVerboseLoggingEnabled) {
                    ...
                    WifiStateMachine.this.handleNetworkDisconnect();
                    if (WifiStateMachine.this.getCurrentState() != WifiStateMachine.this.mFilsState) {
                        WifiStateMachine.this.transitionTo(WifiStateMachine.this.mDisconnectedState);
                        break;
                    }
                    break;
                ...
                case 151553:
                    ...
                    -get93 = WifiStateMachine.this.mWifiConnectionStatistics;
                    -get93.numWifiManagerJoinAttempt++;
                    WifiStateMachine.this.convertToQuotedSSID(config);
                    ...
                    if (netId == -1) {
                        WifiStateMachine.this.setManuConnectConfiguration(config);
                    } else {
                        WifiStateMachine.this.setManuConnectConfiguration(WifiStateMachine.this.mWifiConfigManager.getConfiguredNetwork(netId));
                    }
                    ...
                    WifiStateMachine.this.setManuConnect(true);
                    WifiStateMachine.this.setManuConnectNetId(netId);
                    WifiStateMachine.this.mOppoWifiConnectionAlert.sendConnectNetworkEvent(netId);
                    WifiStateMachine.sManualConnect = true;
                    WifiStateMachine.sNetId = netId;
                    if (WifiStateMachine.this.mWifiNetworkAvailable != null) {
                        WifiStateMachine.this.mWifiNetworkAvailable.setManualConnect(WifiStateMachine.sManualConnect);
                    } else if (WifiStateMachine.this.mWifiNetworkStateTraker != null) {
                        ...
                        WifiStateMachine.this.mWifiNetworkStateTraker.setManualConnect(WifiStateMachine.sManualConnect, false, message.sendingUid);
                ...
                case 151562:
                    ...
                    if (wpsInfo != null) {
                        ...
                        WifiStateMachine.sConnected = false;
                        WifiStateMachine.sConnection = false;
                        WifiStateMachine.sStartWps = true;
                        if (OppoAutoConnectManager.getInstance() != null) {
                            OppoAutoConnectManager.getInstance().handleWpsConnect(true, false);
                        }
                        if (WifiStateMachine.this.mWifiNetworkAvailable != null) {
                            WifiStateMachine.this.mWifiNetworkAvailable.setManualConnect(true);
                        } else if (WifiStateMachine.this.mWifiNetworkStateTraker != null) {
                            WifiStateMachine.this.mWifiNetworkStateTraker.setManualConnect(true, false, 1000);
                        ...
                        if (wpsResult.status != Status.SUCCESS) {
                            ...
                            WifiStateMachine.sConnected = false;
                            WifiStateMachine.sConnection = false;
                            WifiStateMachine.sStartWps = false;
                            if (OppoAutoConnectManager.getInstance() != null) {
    ...
    class ConnectedState extends State {
        ...
        public void enter() {
            ...
            if (!WifiStateMachine.this.mWifiInfo.is5GHz() && (WifiStateMachine.this.mAutoSwitch ^ 1) != 0 && WifiStateMachine.this.mWifiNetworkStateTraker.getAutoSwitch()) {
                WifiStateMachine.this.mWifiNetworkStateTraker.setAutoSwitch(WifiStateMachine.this.mAutoSwitch);
            }
        }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case 131206:
                    ...
                    WifiStateMachine.this.mWifiConfigRestore.isConfigHaveNetwork();
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiStateMachine.CMD_IP_CONFIGURATION_SUCCESSFUL /*131210*/:
                    ...
                    if (WifiStateMachine.this.getCurrentWifiConfiguration() != null) {
                        if (WifiStateMachine.this.mWifiNetworkStateTraker != null && (WifiStateMachine.this.mWifiNetworkStateTraker.getManualConnect() ^ 1) != 0 && WifiStateMachine.this.wifiAssistantForSoftAP() && WifiStateMachine.this.mAutoSwitch && WifiStateMachine.this.getRomUpdateBooleanValue("OPPO_WIFI_ASSISTANT_DETECT_CONNECT", Boolean.valueOf(true)).booleanValue() && WifiStateMachine.this.mNetworkInfo.getDetailedState() != DetailedState.CONNECTED && (WifiStateMachine.this.mNetworkDetectValid ^ 1) != 0) {
    ...
    class ObtainingIpState extends State {
        ...
        public void enter() {
            ...
            if (WifiStateMachine.this.mIsFilsConnection) {
        ...
        public void enter() {
            ...
            if (WifiStateMachine.this.mWifiScanner == null) {
                ...
                synchronized (WifiStateMachine.this.mWifiReqCountLock) {
                    ...
                    WifiConnectivityManager -get94 = WifiStateMachine.this.mWifiConnectivityManager;
                    if (WifiStateMachine.this.mUntrustedReqCount > 0) {
                        z = true;
                    } else {
                        z = false;
                    }
                    -get94.setUntrustedConnectionAllowed(z);
        ...
        protected void releaseNetworkFor(NetworkRequest networkRequest) {
            synchronized (WifiStateMachine.this.mWifiReqCountLock) {
                ...
                if (wifiStateMachine.mConnectionReqCount = wifiStateMachine.mConnectionReqCount - 1 == 0 && WifiStateMachine.this.mWifiConnectivityManager != null) {
                    int -get85 = WifiStateMachine.this.mUntrustedReqCount;
                }
            }
        }
        ...
        public boolean processMessage(Message message) {
            switch (message.what) {
                ...
                case WifiMonitor.NETWORK_CONNECTION_EVENT /*147459*/:
                    ...
                    WifiStateMachine.this.deferMessage(message);
                    WifiStateMachine.sConnection = true;
                    WifiStateMachine.this.transitionTo(WifiStateMachine.this.mDisconnectedState);
                    break;
                case WifiMonitor.NETWORK_DISCONNECTION_EVENT /*147460*/:
                    if (WifiStateMachine.this.mVerboseLoggingEnabled) {
    ...
    /* synthetic */ void lambda$-com_android_server_wifi_WifiStateMachine_13908() {
    ...
    public WifiStateMachine(Context context, FrameworkFacade facade, Looper looper, UserManager userManager, WifiInjector wifiInjector, BackupManagerProxy backupManagerProxy, WifiCountryCode countryCode, WifiNative wifiNative, WrongPasswordNotifier wrongPasswordNotifier) {
        ...
        this.mContext.getContentResolver().registerContentObserver(Global.getUriFor(WIFI_AUTO_CHANGE_ACCESS_POINT), true, new ContentObserver(getHandler()) {
            public void onChange(boolean selfChange) {
                WifiStateMachine.this.mAutoSwitch = Global.getInt(WifiStateMachine.this.mContext.getContentResolver(), WifiStateMachine.WIFI_AUTO_CHANGE_ACCESS_POINT, 1) == 1;
                if (WifiStateMachine.this.mWifiNetworkAvailable != null) {
                    ...
                    if (!WifiStateMachine.this.mAutoSwitch && WifiStateMachine.this.mRotation == 3 && WifiStateMachine.this.mUserSelectSwith && WifiStateMachine.this.mWifiInfo.is5GHz() && WifiStateMachine.this.getRomUpdateBooleanValue("OPPO_WIFI_ASSISTANT_RIGHT_ROTATION", Boolean.valueOf(true)).booleanValue() && SystemProperties.get("debug.gamemode.value", "0").equals("1")) {
                        WifiStateMachine.this.logd("270. gm");
                        return;
                    }
                    WifiStateMachine.this.mWifiNetworkStateTraker.setAutoSwitch(WifiStateMachine.this.mAutoSwitch);
                }
            ...
            public void onChange(boolean selfChange) {
                ...
                if (WifiStateMachine.this.mContext.getPackageManager().hasSystemFeature("oppo.common_center.wlan.assistant")) {
                    ...
                    if (WifiStateMachine.this.mWifiNetworkAvailable == null) {
                        ...
                        OppoWifiNetworkSwitchEnhance -get102 = WifiStateMachine.this.mWifiNetworkAvailable;
                        if (WifiStateMachine.this.mVerboseLoggingEnabled) {
                            i = 1;
                        } else {
                            i = 0;
                        }
                        -get102.enableVerboseLogging(i);
                        wifiStateMachine = WifiStateMachine.this;
                        if (Global.getInt(WifiStateMachine.this.mContext.getContentResolver(), WifiStateMachine.WIFI_AUTO_CHANGE_ACCESS_POINT, 1) != 1) {
                            z = false;
                        }
        ...
        this.mContext.getContentResolver().registerContentObserver(Global.getUriFor(USER_CLICK_WLAN_ASSISTANT_SWITCH), true, new ContentObserver(getHandler()) {
            public void onChange(boolean selfChange) {
                boolean z = true;
                WifiStateMachine wifiStateMachine = WifiStateMachine.this;
                if (Global.getInt(WifiStateMachine.this.mContext.getContentResolver(), WifiStateMachine.USER_CLICK_WLAN_ASSISTANT_SWITCH, -1) != 1) {
                    z = false;
                }
                wifiStateMachine.mUserSelectSwith = z;
                WifiStateMachine.this.logd("onChange mus= " + WifiStateMachine.this.mUserSelectSwith);
            }
        });
        ...
        this.mWifiMonitor.registerHandler(this.mInterfaceName, 147466, getHandler());
        ...
        if (isWlanAssistantEnable()) {
            ...
            int initUserSelectSwitch = Global.getInt(this.mContext.getContentResolver(), USER_CLICK_WLAN_ASSISTANT_SWITCH, -1);
            this.mAutoSwitch = Global.getInt(this.mContext.getContentResolver(), WIFI_AUTO_CHANGE_ACCESS_POINT, 1) == 1;
            this.mUserSelectSwith = this.mAutoSwitch;
            this.mWifiNetworkStateTraker.setAutoSwitch(this.mAutoSwitch);
            Log.d(TAG, "mus = " + this.mUserSelectSwith + ", mas=" + this.mAutoSwitch);
        } else {
    ...
    public void setSupplicantRunning(boolean enable) {
        ...
        sendMessageDelayed(CMD_STOP_SUPPLICANT, (long) (this.mDisconnectDelayDuration * 1000));
    }
    ...
    private void sendNetworkStateChangeBroadcast(String bssid) {
        if (this.mWifiNetworkStateTraker == null || (this.mWifiNetworkStateTraker.getManualConnect() ^ 1) == 0 || !this.mAutoSwitch || this.mNetworkInfo.getDetailedState() == DetailedState.IDLE || this.mNetworkInfo.getDetailedState() == DetailedState.SCANNING || this.mNetworkInfo.getDetailedState() == DetailedState.DISCONNECTED || this.mNetworkInfo.getDetailedState() == DetailedState.FAILED || this.mNetworkInfo.getDetailedState() == DetailedState.BLOCKED || this.mNetworkInfo.getDetailedState() == DetailedState.CONNECTED) {
            Intent intent = new Intent("android.net.wifi.STATE_CHANGE");
            boolean wlanAssistAutoConnect = false;
            boolean replacePending = false;
            if (!(this.mWifiNetworkStateTraker == null || (this.mWifiNetworkStateTraker.getManualConnect() ^ 1) == 0 || !this.mAutoSwitch)) {
    ...
    private boolean setNetworkDetailedState(DetailedState state) {
        ...
        if (!(this.mNetworkInfo.getExtraInfo() == null || this.mWifiInfo.getSSID() == null || (this.mWifiInfo.getSSID().equals(WifiSsid.NONE) ^ 1) == 0 || this.mNetworkInfo.getExtraInfo().equals(this.mWifiInfo.getSSID()))) {
            if (this.mVerboseLoggingEnabled) {
    ...
    public boolean getManuConnect() {
        if (this.mVerboseLoggingEnabled) {
            Log.d(TAG, "getManuConnect=" + sManuConnect);
        }
        return sManuConnect;
    }
    ...
    public void setManuConnectNetId(int id) {
        ...
        sManuConnectNetId = id;
    }

    public int getManuConnectNetId() {
        if (this.mVerboseLoggingEnabled) {
            Log.d(TAG, "getManuConnectNetId=" + sManuConnectNetId);
        }
        return sManuConnectNetId;
    }
    ...
    public void setManuConnectConfiguration(WifiConfiguration config) {
        ...
        sManuConnectConfiguration = config;
    }

    public WifiConfiguration getManuConnectConfiguration() {
        if (sManuConnectConfiguration != null) {
            if (this.mVerboseLoggingEnabled) {
                Log.d(TAG, "getManuConnectConfiguration netId=" + sManuConnectConfiguration.networkId + " ssid=" + sManuConnectConfiguration.SSID + " bssid=" + sManuConnectConfiguration.BSSID);
            }
        } else if (this.mVerboseLoggingEnabled) {
            Log.d(TAG, "getManuConnectConfiguration = null");
        }
        return sManuConnectConfiguration;
    }
    ...
    private SupplicantState handleSupplicantStateChange(Message message) {
        ...
        } else if (sConnection && stateChangeResult.networkId == -1) {
    ...
    private void handleNetworkDisconnect() {
        ...
        if (sManualConnect && sConnected) {
            if (this.mVerboseLoggingEnabled) {
                Log.d(TAG, "handleNetworkDisconnect set sConnected false ");
            }
            sConnected = false;
            if (OppoAutoConnectManager.getInstance() != null) {
                OppoAutoConnectManager.getInstance().handleWpsConnect(false, true);
        ...
        if ((sNetId == this.mLastNetworkId && this.mConnectedId == this.mLastNetworkId && (sManualConnect ^ 1) != 0 && (sStartWps ^ 1) != 0) || sConnected) {
            if (this.mWifiNetworkAvailable != null) {
        ...
        if (getCurrentState() != this.mFilsState) {
            stopIpManager();
        }
        this.mWifiScoreReport.reset();
    ...
    public boolean startRxSensTest(WifiConfiguration config, String ip) {
        ...
        NetworkUpdateResult result = this.mWifiConfigManager.addOrUpdateNetwork(config, 1000);
        if (!result.isSuccess()) {
            return false;
        }
    ...
    private void sendConnectModeChangeBroadcast(boolean manualConnect) {
    ...
    private void tryUpdateFewApNum() {
        this.fewApNum = getRomUpdateIntegerValue("OPPO_FEW_AP_NUM", Integer.valueOf(5)).intValue();
    }
    ...
    private void oppoStatisticsForScanFewAP() {
        ...
        if (!(srList == null || srList.size() <= 0 || tmpAllRssiAndChannel == null)) {
            for (ScanResult sr : srList) {
        ...
        Location lastLocation = ((LocationManager) this.mContext.getSystemService("location")).getLastLocation();
        if (lastLocation != null) {
            String latitude = Double.toString(lastLocation.getLatitude());
            location = latitude + "," + Double.toString(lastLocation.getLongitude());
        }
        map.put(SCAN_NUM, scanNum);
        map.put(SCAN_LOCATION, location);
        map.put(SCAN_RSSI_CHANNEL, allRssiAndChannel);
        OppoStatistics.onCommon(this.mContext, "wifi_fool_proof", "scan_few_ap", map, false);
    }

    private void checkScanFewAP() {
        String stateName = getCurrentState().getName();
        if ("DisconnectedState".equalsIgnoreCase(stateName) || "ScanModeState".equalsIgnoreCase(stateName)) {
            tryUpdateFewApNum();
            if ((this.mScanResults == null || this.mScanResults.size() < this.fewApNum) && isInWirelessSettings() && (isScanConfigedAp() ^ 1) != 0) {
    ...
    public void checkTurnOnWifi(String packageName, boolean enable) {
        if (!this.scanFewApFlag) {
            this.turnOnWifiFlag = false;
        } else if (!TextUtils.isEmpty(packageName) && packageName.equals(WIRELESS_SETTINGS_PACKAGE_NAME) && enable) {
    ...
    private boolean isInWirelessSettings() {
        ...
        if (runningTaskInfos != null && runningTaskInfos.size() > 0) {
            ...
            if (!(info == null || info.topActivity == null)) {
                ...
                return !TextUtils.isEmpty(pkg) && pkg.equals(WIRELESS_SETTINGS_PACKAGE_NAME);
            }
        }
    }
    ...
    private boolean isScanConfigedAp() {
        List<ScanResult> srList = syncGetScanResultsList();
        List<WifiConfiguration> wcList = this.mWifiConfigManager.getConfiguredNetworks();
