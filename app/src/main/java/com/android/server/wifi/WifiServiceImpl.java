...
public class WifiServiceImpl extends OppoWifiServiceImpl {
    ...
    public WifiServiceImpl(Context context, WifiInjector wifiInjector, AsyncChannel asyncChannel) {
        ...
        if (this.mWifiInjector.getOppoWifiSharingManager().isWifiSharingSupported()) {
            this.mWifiStateMachine.setNewSapInterface(this.mWifiApConfigStore.getSapInterface());
            this.mWifiController.setStaSoftApConcurrencyForSharing(false);
        }
    ...
    public synchronized boolean setWifiEnabled(String packageName, boolean enable) throws RemoteException {
        ...
        if (!this.mSettingsStore.isAirplaneModeOn() || (isFromSettings ^ 1) == 0) {
            if (!(this.mWifiStateMachine.syncGetWifiApState() != 11) || (isFromSettings ^ 1) == 0) {
                if (isStrictOpEnable()) {
                ...
                try {
                    if (this.mSettingsStore.handleWifiToggled(enable)) {
                        ...
                        if (this.mIsControllerStarted) {
                            ...
                            if (this.mWifiInjector.getOppoWifiSharingManager().isWifiSharingSupported()) {
                                int apEnabledState = getWifiApEnabledState();
                            ...
                            this.mWifiStateMachine.checkTurnOnWifi(packageName, enable);
                            return true;
                        }
    ...
    public int getWifiApEnabledState() {
        ...
        OppoWifiSharingManager oppoSharingManager = this.mWifiInjector.getOppoWifiSharingManager();
        if (oppoSharingManager.isWifiSharingTethering()) {
            return oppoSharingManager.transformApState2SharingState(this.mWifiStateMachine.syncGetWifiApState());
        }
    ...
    /* synthetic */ void lambda$-com_android_server_wifi_WifiServiceImpl_48609(String ifaceName, int mode) {
    ...
    public boolean startSoftAp(WifiConfiguration wifiConfig) {
        ...
        synchronized (this.mLocalOnlyHotspotRequests) {
            ...
            boolean checkAndStartSharingOrSoftapInternal;
            if (this.mWifiInjector.getOppoWifiSharingManager().isWifiSharingSupported()) {
                checkAndStartSharingOrSoftapInternal = checkAndStartSharingOrSoftapInternal(wifiConfig);
                return checkAndStartSharingOrSoftapInternal;
            }
            checkAndStartSharingOrSoftapInternal = startSoftApInternal(wifiConfig, 1);
            return checkAndStartSharingOrSoftapInternal;
        }
    }
    ...
    public void enableVerboseLogging(int verbose) {
        ...
        this.mWifiInjector.getOppoWifiSharingManager().enableVerboseLogging(verbose);
    ...
    public void setWifiSharingConfiguration(WifiConfiguration wifiConfig) {
        enforceChangePermission();
        int uid = Binder.getCallingUid();
        if (this.mWifiPermissionsUtil.checkConfigOverridePermission(uid)) {
    ...
    private boolean checkAndStartSharingOrSoftapInternal(WifiConfiguration wifiConfig) {
        OppoWifiSharingManager sharingManager = this.mWifiInjector.getOppoWifiSharingManager();
        boolean sharingSupported = sharingManager.isWifiSharingSupported();
        Log.d(TAG, "checkAndStartSharingOrSoftapInternal");
        if (!sharingSupported || wifiConfig == null || wifiConfig.networkId != -1000) {
            disableWifiSharing();
            sharingManager.setWifiTetheringType(0);
            sharingManager.setWifiClosedByUser(true);
        } else if (this.mWifiStateMachine.isDisconnected()) {
            Log.e(TAG, "start wifisharing false, due to wifi disconnected.");
            return false;
        } else if (this.mWifiController.isSharingEnabing()) {
            Log.e(TAG, "start wifisharing false, due to already enabling.");
            return false;
        } else {
            this.mWifiStateMachine.stopP2pConnectInternal();
            sharingManager.setWifiTetheringType(4);
            wifiConfig = this.mWifiApConfigStore.getSharingConfiguration();
            this.mWifiController.setStaSoftApConcurrencyForSharing(true);
        }
        return startSoftApInternal(wifiConfig, 1);
    }

    private void disableWifiSharing() {
