...
public final class DreamManagerService extends SystemService {
    ...
    private boolean isShouldGoAod() {
        ...
        int mAodEnableImediate = Secure.getIntForUser(resolver, "Setting_AodEnableImediate", 0, -2);
        int mAodUserSetTime = Secure.getIntForUser(resolver, "Setting_AodUserSetTime", 0, -2);
        Slog.i(TAG, "check mAodEnableImediate =" + mAodEnableImediate);
        Slog.i(TAG, "check mAodUserSetTime =" + mAodUserSetTime);
        if (mAodEnableImediate == 1 || mAodUserSetTime == 1) {
            return true;
        }
        return false;
    }
    ...
    private ComponentName getDozeComponent(int userId) {
        return ComponentName.unflattenFromString("com.oppo.aod/.OppoAodService");
    }
    ...
    /* synthetic */ void lambda$-com_android_server_dreams_DreamManagerService_16642(Binder newToken, ComponentName name, boolean isTest, boolean canDoze, int userId, WakeLock wakeLock) {
