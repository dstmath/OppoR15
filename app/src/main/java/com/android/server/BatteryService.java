...
public final class BatteryService extends SystemService {
    ...
    private static final int NS_LEVEL_UPDATE_THRESH = 2;
    static final int OPTION_FORCE_UPDATE = 1;
    private static final String TAG = BatteryService.class.getSimpleName();
    private static final String TAG_LED = "BatteryLed";
    private static int mNsLastBattStatus = 1;
    private static int mNsLastRecordLevel = 0;
    ...
    public BatteryService(Context context) {
        ...
        try {
            this.mHwStatusIsSet = Integer.parseInt(SystemProperties.get("persist.sys.hw_status"));
    ...
    public void onBootPhase(int phase) {
        ...
        if (phase == SystemService.PHASE_ACTIVITY_MANAGER_READY) {
            ...
            obj = this.mLock;
            synchronized (obj) {
                AnonymousClass4 anonymousClass4 = new ContentObserver(this.mHandler) {
                    public void onChange(boolean selfChange) {
                        synchronized (BatteryService.this.mLock) {
                            BatteryService.this.mLed.updateLightsLocked();
                        }
    ...
    private void update(BatteryProperties props) {
        synchronized (this.mLock) {
            ...
            if (this.mUpdatesStopped) {
                ...
                sendNsBattStatusLocked(props);
            }
        }
    }

    private void sendNsBattStatusLocked(BatteryProperties props) {
        int i = 0;
        if (NeuronSystemManager.isEnable()) {
            int battLevel = props.batteryLevel;
            int battStatus = props.batteryStatus;
            if (battLevel - mNsLastRecordLevel >= 2 || battLevel - mNsLastRecordLevel <= -2 || battStatus != mNsLastBattStatus) {
                int i2;
                ...
                charger |= i2;
                if (props.chargerWirelessOnline) {
                    i = 4;
                }
                charger |= i;
                ContentValues contentValues = new ContentValues();
                contentValues.put("status", Integer.valueOf(battStatus));
                contentValues.put("charger", Integer.valueOf(charger));
                contentValues.put("level", Integer.valueOf(battLevel));
                contentValues.put("date", Long.valueOf(System.currentTimeMillis()));
                NeuronSystemManager.getInstance().publishEvent(5, contentValues);
                mNsLastRecordLevel = battLevel;
            }
            mNsLastBattStatus = props.batteryStatus;
        }
    }
