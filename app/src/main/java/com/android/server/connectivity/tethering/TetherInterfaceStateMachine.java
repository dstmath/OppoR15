...
public class TetherInterfaceStateMachine extends StateMachine {
    ...
    private static final int WIFI_SHARNG_HOST_IFACE_PREFIX_LENGTH = 24;
    ...
    private boolean configureIPv4(boolean enabled) {
        ...
        if (this.mInterfaceType == 1) {
            ...
            prefixLen = 24;
        } else if (this.mInterfaceType != 4) {
            return true;
        } else {
            ipAsString = getWifiSharingIfaceAddr();
            prefixLen = 24;
        }
        try {
            ...
            if (this.mInterfaceType == 0 || this.mInterfaceType == 4) {
    ...
    private String getWifiSharingIfaceAddr() {
        try {
            return intToStringGatewayForSharing(Stub.asInterface(ServiceManager.getService("wifi")).getDhcpInfo().gateway);
        } catch (RemoteException e) {
    ...
    public String intToStringGatewayForSharing(int hostAddress) {
        return NetworkUtils.intToInetAddress(hostAddress).getHostAddress().equals(WIFI_HOST_IFACE_ADDR) ? WIFI_SHARNG_HOST_IFACE_ADDR : WIFI_HOST_IFACE_ADDR;
    }
}
