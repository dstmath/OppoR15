...
public class NetworkMonitor extends StateMachine {
        ...
        public void enter() {
            ...
            if (NetworkMonitor.this.mArpPeer != null) {
                ...
                if (NetworkMonitor.this.needToCheckGateway()) {
                    NetworkMonitor.this.probeGateway();
                    if (NetworkMonitor.this.mArpPeer.fetchGatewayMacFromRoute() == null) {
                        delay = 2000;
                    }
                }
                networkMonitor = NetworkMonitor.this;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case NetworkMonitor.CMD_REEVALUATE /*532486*/:
                    ...
                    NetworkMonitor.this.log("mSkipNetworkValidation =" + NetworkMonitor.mSkipNetworkValidation);
                    ...
                    if (probeResult.isSuccessful()) {
                        ...
                        if (!(NetworkMonitor.this.mNetworkAgentInfo == null || NetworkMonitor.this.mNetworkAgentInfo.networkInfo == null || NetworkMonitor.this.mNetworkAgentInfo.networkInfo.getType() != 1)) {
                            if (NetworkMonitor.this.shallAccountForDupGateway() && this.mAttempts == 1) {
                                ...
                                return true;
                            } else if (NetworkMonitor.this.hasWlanAssistant() && (NetworkMonitor.this.haveSendmsg ^ 1) != 0) {
    ...
    protected NetworkMonitor(Context context, Handler handler, NetworkAgentInfo networkAgentInfo, NetworkRequest defaultRequest, IpConnectivityLog logger) {
        ...
        this.mPublicServers = new String[]{"https://m.baidu.com", "http://info.3g.qq.com", "https://sina.cn", "https://m.sohu.com"};
        ...
        if (this.mNetworkAgentInfo == null || this.mNetworkAgentInfo.networkInfo == null || this.mNetworkAgentInfo.networkInfo.getType() != 1) {
            ...
            this.mCaptivePortalHttpsUrl = makeURL((String) publicServers.get(0));
            this.mCaptivePortalHttpUrl = makeURL("http://" + ((String) updatedServers.get(index % 2)) + "/generate_204");
        }
        ...
        try {
            ...
            if (!TextUtils.isEmpty(hostToResolve)) {
                ...
                if (addresses.length == 1 && sb.toString().equals(addresses[0].getHostAddress())) {
                    ...
                    CaptivePortalProbeResult captivePortalProbeResult = new CaptivePortalProbeResult(VoldResponseCode.OpFailedVolNotMounted, null, url.toString());
    ...
    private boolean isNeedValidateOperator() {
        ...
        String mcc = colorOSTelephonyManager.getNetworkOperatorGemini(colorOSTelephonyManager.colorGetDataSubscription());
        if (TextUtils.isEmpty(mcc)) {
            log("isNeedValidateOperator() mcc is empty, return default value false");
        } else {
    ...
    private void probeGateway() {
        this.mArpPeer.doDupArp(this.mInterfaceName, this.mIPv4Self, this.mIPv4Gateway);
    }
