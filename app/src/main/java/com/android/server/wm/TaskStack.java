...
public class TaskStack extends WindowContainer<Task> implements DimLayerUser, BoundsAnimationTarget {
    ...
    void onDisplayChanged(DisplayContent dc) {
        ...
        } else if (this.mStackId == 7) {
            int width = this.mDisplayContent.mInitialDisplayWidth;
            int height = this.mDisplayContent.mInitialDisplayHeight;
            if (width > height) {
                int temp = width;
                width = height;
                height = temp;
            }
            bounds = new Rect(height, 0, height + height, height);
