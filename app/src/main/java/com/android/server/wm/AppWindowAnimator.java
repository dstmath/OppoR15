...
public class AppWindowAnimator {
    ...
    public void setAnimation(Animation anim, int width, int height, int parentWidth, int parentHeight, boolean skipFirstFrame, int stackClip, int transit, int transitFlags) {
        ...
        float transitionAnimationScale = this.mService.getTransitionAnimationScaleLocked();
        Task atask = this.mAppToken.getTask();
        if (!(atask == null || atask.mStack == null || atask.mStack.mStackId != 7)) {
            transitionAnimationScale = OppoBrightUtils.MIN_LUX_LIMITI;
        }
        anim.scaleCurrentDuration(transitionAnimationScale);
