...
class TaskSnapshotController {
    ...
    private TaskSnapshot snapshotTask(Task task) {
        ...
        Rect insetRect = minRect(mainWindow.mContentInsets, mainWindow.mStableInsets);
        if (OppoScreenDragUtil.isOffsetState()) {
            float scale = OppoScreenDragUtil.getScale();
            if (OppoScreenDragUtil.getOffsetX() > 0) {
                insetRect = new Rect((int) ((((float) buffer.getWidth()) * (1.0f - scale)) + (((float) insetRect.left) * scale)), (int) ((((float) buffer.getHeight()) * (1.0f - scale)) + (((float) insetRect.top) * scale)), insetRect.right, (int) (((float) insetRect.bottom) * scale));
            } else {
                insetRect = new Rect(insetRect.left, (int) ((((float) buffer.getHeight()) * (1.0f - scale)) + (((float) insetRect.top) * scale)), (int) ((((float) buffer.getWidth()) * (1.0f - scale)) + (((float) insetRect.right) * scale)), (int) (((float) insetRect.bottom) * scale));
            }
        }
        return new TaskSnapshot(buffer, top.getConfiguration().orientation, insetRect, isLowRamDevice, scaleFraction);
    }
    ...
    /* synthetic */ void lambda$-com_android_server_wm_TaskSnapshotController_15491(ScreenOffListener listener) {
        try {
    ...
    /* synthetic */ void lambda$-com_android_server_wm_TaskSnapshotController_15656(Task task) {
        if (task.isVisible()) {
