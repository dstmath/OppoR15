...
class WindowSurfaceController {
    ...
    private void hideSurface() {
        if (this.mSurfaceControl != null) {
            setShown(false);
            try {
                if (!((!WindowManagerService.DEBUG_WMS && !WindowManagerDebugConfig.SHOW_TRANSACTIONS) || DetectBlack.getInstance().isLastCheckBlack() || (DetectBlack.getInstance().checkHasSurface(this) ^ 1) == 0)) {
    ...
    private boolean showSurface() {
        try {
            ...
            if ((WindowManagerService.DEBUG_WMS || WindowManagerDebugConfig.SHOW_TRANSACTIONS) && DetectBlack.getInstance().isLastCheckBlack() && DetectBlack.getInstance().isWindowSurfaceContrlInScreenCenter(this) && DetectBlack.getInstance().checkHasSurface()) {
