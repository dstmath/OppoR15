...
class TelephonyRegistry extends Stub {
    ...
    public void notifyServiceStateForPhoneId(int phoneId, int subId, ServiceState state) {
        if (checkNotifyPermission("notifyServiceState()")) {
            ...
            notifyServiceStateToNeuron(state);
        }
    }
    ...
    public void notifySignalStrengthForPhoneId(int phoneId, int subId, SignalStrength signalStrength) {
        if (checkNotifyPermission("notifySignalStrength()")) {
            ...
            notifySignalStrengthToNeuron(signalStrength);
        }
    }
    ...
    private void notifySignalStrengthToNeuron(SignalStrength signalStrength) {
        if (NeuronSystemManager.isEnable() && NeuronSystemManager.isHookEnable(2)) {
            ContentValues contentValues = new ContentValues();
            contentValues.put("signal_strength", Integer.valueOf(signalStrength.getLevel()));
            NeuronSystemManager.getInstance().publishEvent(13, contentValues);
        }
    }

    private void notifyServiceStateToNeuron(ServiceState state) {
        if (NeuronSystemManager.isEnable()) {
            int csType = state.getVoiceNetworkType();
            int psType = state.getDataNetworkType();
            ContentValues contentValues = new ContentValues();
            contentValues.put("PLMN", state.getDataOperatorNumeric());
            contentValues.put("network_type", Integer.valueOf(csType));
            contentValues.put("cs_time", Long.valueOf(System.currentTimeMillis()));
            contentValues.put("mobile_data_type", Integer.valueOf(psType));
            contentValues.put("mobile_data_class", Integer.valueOf(TelephonyManager.getNetworkClass(psType)));
            contentValues.put("ps_time", Long.valueOf(System.currentTimeMillis()));
            NeuronSystemManager.getInstance().publishEvent(12, contentValues);
        }
    }
}
