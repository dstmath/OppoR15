        ...
        private int subParseZoneStringFromICU(String text, int start, CalendarBuilder calb) {
            ...
            if (tzId == null) {
                ...
                if (set.contains(bestMatch.mzID())) {
                    ...
                    ULocale uLocale2 = ULocale.forLocale(this.locale);
                    String region2 = uLocale2.getCountry();
                    if (region2.length() == 0) {
                        region2 = ULocale.addLikelySubtags(uLocale2).getCountry();
                    }
                    tzId = tzNames.getReferenceZoneID(bestMatch.mzID(), region2);
                }
            }
            newTimeZone = TimeZone.getTimeZone(tzId);
            if (currentTimeZoneID.equals(tzId)) {
