                ...
                if (q == null) {
                    if (p.prev != p) {
                        ...
                        if (hops < 2 || (isFirst | isLast) == 0) {
                            ...
                            skipDeletedPredecessors(activeSucc);
                            if ((isFirst | isLast) != 0 && activePred.next == activeSucc && activeSucc.prev == activePred && (isFirst ? activePred.prev != null : activePred.item == null) && (isLast ? activeSucc.next != null : activeSucc.item == null)) {
                                Node prevTerminator22;
                                Node<E> prevTerminator222;
                                updateHead();
                                updateTail();
                                if (isFirst) {
                                    prevTerminator222 = prevTerminator();
                                } else {
                                    prevTerminator222 = x;
                                }
                                x.lazySetPrev(prevTerminator222);
                                if (isLast) {
                                    prevTerminator222 = nextTerminator();
                                } else {
                                    prevTerminator222 = x;
                                }
                                x.lazySetNext(prevTerminator222);
                            }
                        } else {
                            return;
                        }
